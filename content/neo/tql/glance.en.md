...
- key : generally an auto-generated sequential integer, similar to *ROWNUM* in query results
- value : a tuple that contains the actual data fields
...
- `SQL()` : produces records by querying the Machbase-neo database or even external (bridged) databases using the given SQL statement
- `FAKE()` : generates artificial data for testing purposes
- `CSV()` : reads data from CSV files
- `BYTES()` : reads arbitrary binary data from the file system, client's HTTP request, or MQTT payload

![tql_src](../img/tql_src.jpg)

### SINK

- `INSERT()` : writes the incoming records to the Machbase-neo database
- `CHART()` : renders a chart with the incoming records
- `JSON()`, `CSV()` : encode the incoming data into their respective formats, making it easier to integrate with other applications or display the data in a user-friendly manner
...
They allow you to perform various operations such as mathematical calculations, string manipulations, data format conversions, and interactions with external systems.
...
Copy and paste the sample TQL code into the TQL editor, then click the ▶︎ icon on the top left of the editor.
It displays a chart as shown in the images below, representing a wave with a frequency of 1.5 Hz and an amplitude of 1.0.
...
Invoke `hello.tql` with `curl` from the terminal to receive the result in JSON format.

```sh
curl -o - http://127.0.0.1:5654/db/tql/hello.tql
```

The JSON response with HTTP headers looks like this.
