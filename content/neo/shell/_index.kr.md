...
### API token으로 SSH 인증하기

{{< neo_since ver="8.7.1" />}}

machbase-neo는 SSH 비밀번호 입력란에 API token을 전달해 인증하는 방법도 지원합니다. API token은 일반 DB 비밀번호 대신 사용되며, 인증에 성공하면 token 소유자의 DB 사용자 권한으로 SSH 셸이 실행됩니다.

API token을 발급한 뒤 SSH 접속을 실행하고, 비밀번호 입력 프롬프트에 token을 입력합니다.
...
SSH 사용자명의 기본 사용자 부분은 API token 소유자와 일치해야 합니다(대소문자는 구분하지 않습니다). 예를 들어 `sys`로 접속하면 `sys` 사용자의 token을 입력해야 합니다. API token은 비밀번호와 동일한 자격증명이므로 명령줄 인자, 셸 히스토리, 로그에 노출되지 않도록 주의합니다.
