...
산업 현장의 센서 데이터는 “산업용 빅데이터”, “Smart-X”와 같은 이름으로 불릴 만큼 기하급수적으로 증가하고 있습니다. 방대한 시계열 데이터를 지속적으로 저장하려면 데이터 보관, 재해 복구, 과거 데이터 분석을 위한 견고한 메커니즘이 필수입니다. 그러나 전통적인 백업/복구 방식은 백업 범위를 유연하게 조절하기 어렵고, 복구에 시간이 많이 걸리며, 전체 복구를 수행하기 전에는 백업 데이터를 조회할 수 없다는 한계가 있습니다. 이러한 복구 작업은 운영을 방해하고 자원도 많이 소모합니다.

Machbase는 이러한 문제를 해결하기 위해 시계열 워크로드에 특화된 **백업(Backup)** 과 **마운트(Mount)** 기능을 제공합니다. 전체·증분·시간 범위·테이블 단위 백업을 지원하며, 특히 시간 소모가 큰 복구 과정 없이 백업 데이터를 온라인 상태에서 즉시 읽기 전용으로 부착할 수 있는 마운트 기능이 핵심입니다.

## 핵심 개념

- **Backup**: 데이터베이스 전체 혹은 특정 테이블/시간 범위를 외부 스토리지에 물리적으로 복사합니다. Machbase 백업은 데이터와 메타데이터 파일이 포함된 디렉터리 구조로 저장됩니다.
- **Restore**: 백업에서 데이터베이스를 복구합니다. 주로 재해 복구나 복제 환경 구축에 사용합니다. `machbase-neo restore`는 비어 있는 `dbs` 디렉터리를 요구하며, 기존 데이터가 있으면 실행을 거부합니다.
- **Mount**: 백업 디렉터리를 실행 중인 Machbase 인스턴스에 읽기 전용 데이터베이스로 연결합니다. 오래 걸리는 복구 없이도 백업 시점의 “화석화된” 데이터를 즉시 조회할 수 있습니다.
- **Unmount**: 마운트된 백업 데이터를 분리합니다. 백업 파일 자체는 삭제하지 않습니다.
- **Live Data**: 현재 운영 중인 Machbase 인스턴스의 데이터로, 실시간으로 읽기와 쓰기가 이루어집니다.
...
- `INTO DISK = 'path/...'`: 백업 파일이 생성될 대상 디렉터리를 지정합니다. 절대 경로 또는 `$MACHBASE_HOME/dbs` 기준 상대 경로를 사용할 수 있습니다. 백업 과정에서 만들 수 있는 새 경로를 지정하십시오. 디렉터리가 없으면 자동으로 생성됩니다.

**참고 사항**
- 백업을 시작한 시점의 데이터를 캡처합니다.
...
- `AFTER 'path/...'`: 백업 체인의 *직전* 백업(전체 또는 증분) 디렉터리 경로를 지정합니다. 이 경로는 반드시 존재하고 접근 가능해야 합니다.
- `INTO DISK = 'path/...'`: *새* 증분 백업이 저장될 대상 디렉터리를 지정합니다.

**참고 사항**
- 주로 로그(Log) 테이블과 태그(Tag) 테이블처럼 append 작업이 많은 경우에 적합합니다.
- Lookup 테이블은 append 이외의 방식으로도 변경될 수 있으므로 증분 백업이라도 항상 전체를 백업합니다.
- 직전 백업 디렉터리가 온전하게 존재해야 합니다.

### 시간 범위 백업 (데이터베이스 또는 테이블)

특정 기간의 데이터를 백업합니다. 시계열 데이터를 월별이나 분기별처럼 관리하기 쉬운 단위로 나누어 보관할 때 유용합니다.
...
- `FROM time_expression_start`: 백업 시작 시각(포함)을 지정합니다. 예: `TO_DATE('2024-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')`
...
| 경로/파일 | 설명 |
|:--|:--|
| `<path>/backup.dat` | 백업 상태와 관리 정보입니다. Neo는 이 파일로 백업을 식별합니다. |
| `<path>/backup.trc` | 백업 처리 과정의 트레이스 파일입니다. |
| 그 밖의 생성 파일 | 데이터, 메타데이터, 인덱스 등 복구와 마운트에 필요한 파일입니다. 백업 디렉터리 전체를 보존하십시오. |

## 마운트(Mount) 작업

마운트 작업은 백업 디렉터리를 실행 중인 Machbase 인스턴스에 읽기 전용 데이터베이스로 연결합니다. 복구 없이도 백업 데이터를 즉시 조회할 수 있습니다.
...
- `'/path/to/backup_directory'`: 마운트할 백업 디렉터리 경로입니다. `BACKUP ... INTO DISK`로 생성한 디렉터리를 지정합니다.
- `mount_name`: 마운트한 데이터베이스에 붙이는 별칭입니다. 마운트된 데이터를 조회할 때는 이 이름을 객체 이름 앞에 붙여 `mount_name.user_name.table_name` 형식으로 지정합니다.

`mount_name`을 서로 다르게 지정하면 여러 백업을 동시에 마운트할 수 있습니다.

### 마운트한 데이터 조회

마운트된 백업의 테이블에 접근하려면 테이블 이름 앞에 마운트 이름과 원래 스키마 소유자(일반 테이블은 보통 `sys`)를 붙여야 합니다.
...
마운트된 백업 데이터베이스를 분리합니다. 마운트를 해제하면 더 이상 마운트 이름으로 해당 백업 데이터를 조회할 수 없습니다. 백업 파일 자체는 디스크에서 삭제되지 않습니다.
...
`machbase-neo restore`는 중지된 인스턴스의 데이터 디렉터리에 백업을 복구합니다. 주로 재해 복구나 동일한 인스턴스를 구성할 때 사용합니다. 대상 홈 디렉터리는 미리 존재해야 합니다. `dbs` 디렉터리가 없으면 새로 만들지만, 비어 있지 않으면 오류가 발생합니다. 기존 데이터를 자동으로 덮어쓰지는 않습니다.

```bash
machbase-neo restore --data <machbase_home_dir> <path/to/backup_directory>
```

- `--data <machbase_home_dir>`: 복구 대상 Machbase 인스턴스의 `$MACHBASE_HOME` 디렉터리를 지정합니다.
- `<path/to/backup_directory>`: 복구할 백업 디렉터리 경로입니다.
  - 전체 복구일 경우 전체 백업 디렉터리 경로를 지정합니다.
  - 증분 백업을 포함해 복구할 경우 반드시 체인의 **마지막 증분 백업** 디렉터리를 지정해야 합니다. 복구 과정에서 체인의 이전 백업(전체 백업과 중간 증분 백업)을 자동으로 찾아 사용합니다.

**주의 사항**
- 복구는 오프라인 작업입니다. 실행하기 전에 대상 인스턴스를 중지하십시오.
- 의도하지 않은 데이터 손실을 막으려면 대상 `$MACHBASE_HOME`을 확인하고 기존 데이터를 따로 보존하십시오. `dbs`가 비어 있지 않으면 복구할 수 없습니다.
...
- **인덱스 보존**: 백업에 포함된 시계열 인덱스를 그대로 활용하므로 마운트한 데이터도 라이브 데이터와 비슷한 속도로 조회할 수 있습니다.
- **롤업(Rollup) 구조 유지**: 백업 시점의 롤업 테이블도 함께 보존되고 마운트를 통해 조회할 수 있으므로, 라이브 데이터와 과거 데이터를 같은 방식으로 통계 분석할 수 있습니다.
- **온라인 작업**: 마운트/언마운트는 운영 데이터베이스를 중단하지 않고 수행할 수 있습니다.
- **자원 효율성**: 단순 조회를 위해 전체 복구에 필요한 디스크 I/O, CPU 자원을 소비하지 않아도 됩니다.

### 유의사항

- **읽기 전용**: 마운트된 데이터베이스는 읽기 전용입니다. `INSERT`, `UPDATE`, `DELETE`와 DDL은 실행할 수 없습니다. 변경하려면 `RESTORE`가 필요합니다.
- **통계 뷰 제공 여부**: 실시간 통계 뷰(`v$...`)는 마운트된 정적 데이터에 적용되지 않을 수 있습니다. 건수나 집계 값이 필요하면 데이터를 직접 질의하십시오.
- **파일 시스템 권한**: 마운트하려면 Machbase 서버 프로세스가 백업 디렉터리에 읽기 권한을 가져야 합니다.

## 활용 예시

이 절에서는 백업과 마운트를 활용하는 실제 시나리오를 소개합니다.
