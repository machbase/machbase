...
従来のセンサーデータモデルは、多くの場合、各行が1つのタイムスタンプを表し、各列が異なるセンサーの測定値（タグ）に対応する、CSVのような横持ち形式です。
...
タグテーブルは内部で、タグの属性を表すメタデータと実際の時系列データを分けて管理します。
...
| 型       | 説明                      | 範囲・表現                                          | NULLの表現           |
| :------- | :------------------------------- | :-------------------------------------------------------------- | :---------------------------- |
| `SHORT`    | 16ビット符号付き整数            | -32767 〜 32767                                                 | -32768                        |
| `USHORT`   | 16ビット符号なし整数          | 0 〜 65534                                                      | 65535                         |
| `INTEGER`  | 32ビット符号付き整数            | -2147483647 〜 2147483647                                       | -2147483648                   |
| `UINTEGER` | 32ビット符号なし整数          | 0 〜 4294967294                                                 | 4294967295                    |
| `LONG`     | 64ビット符号付き整数            | -9223372036854775807 〜 9223372036854775807                     | -9223372036854775808          |
| `ULONG`    | 64ビット符号なし整数          | 0 〜 18446744073709551614                                      | 18446744073709551615          |
| `FLOAT`    | 32ビット浮動小数点数            | ±1.175494e-38 〜 ±3.402823e+38                                  | 3.402823466e+38               |
| `DOUBLE`   | 64ビット浮動小数点数            | ±2.225074e-308 〜 ±1.797693e+308                                | 1.7976931348623158e+308       |
| `DATETIME` | 日時（ナノ秒精度）| 1970-01-01 00:00:00 000:000:000 UTC以降                        | N/A                           |
| `VARCHAR`  | 可変長文字列（UTF-8） | 1バイト〜32KB（32767バイト）                                    | NULL                          |
| `IPV4`     | IPv4アドレス                     | "0.0.0.0" 〜 "255.255.255.255"                                  | NULL                          |
| `IPV6`     | IPv6アドレス                     | "::" 〜 "FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF:FFFF"               | NULL                          |
| `JSON`     | JSONデータ型                   | データ長：1バイト〜32KB。パス長：1〜512文字 | NULL                          |

**注意：** タグテーブルは、`TEXT`データ型には**対応していません**。
...
タグテーブル（例：`MYTAG`）を作成すると、内部で以下の関連オブジェクトが作成・管理されます。
...
メタデータは`_tableName_META`に保存され、仮想タグテーブルの検索時に効率的に結合できるよう、通常はメモリ上に保持されます。`name`列が、メタデータ属性と時系列データを関連付ける一意のキーです。
...
タグテーブルのデータ削除は、主に時刻またはタグ単位で行います。appendに最適化した構造のため、通常は個別レコードの更新や個別削除に対応していません。
...
タグテーブルには、`name`列と`time`列に自動で作成される、最適化された**内部インデックス**があります。このインデックスが、一般的な時系列クエリの性能を支えます。結果を時刻順に並べるには、`ORDER BY time`を指定してください。
...
**課題**：`name`や`time`の条件がなく、`value`や追加データ列*だけ*で絞り込むクエリは、主な内部インデックスを有効に使えません。
...
*   **ローカルな構造**：外部インデックスは、通常はデータパーティションごとにローカルに分割されます。データ量が増えると、外部インデックスを使っても検索性能が低下する場合がありますが、インデックスなしの全スキャンより大幅に効率的です。
...
Machbase Neoは、性能要件やクライアント環境に応じて、タグテーブルにデータを取り込む複数の方法を提供します。
...
3.  **REST API：**
    ...
    *   データ取り込み用のエンドポイントは`append`メソッドパラメーターに対応しており、内部で効率的なAppendプロトコルを使用します。
...
*   **外部インデックス**：時刻条件なしで、データ列や値列*だけ*で頻繁に絞り込む場合にのみ作成してください。ストレージ使用量と取り込みのオーバーヘッドが増えます。
...
## 基本構成と取り込みの例 {#basic-configuration-and-ingestion-example}

以下は、タグごとに`factory`と`equipment`の属性を持つ`vibration`テーブルの例です。
...
### メタデータ列の追加と登録 {#adding-and-registering-metadata-columns}

タグごとの属性は、メタテーブル`_vibration_meta`に保存します。構造の変更には`ALTER TABLE`を使用します。メタデータ列は`ADD COLUMN`で追加し、`DROP COLUMN`で削除できます。
...
### データの取り込み {#loading-data}

#### importツール {#import-tool}

CSVのフィールドは、メタデータ列を含むテーブルの列順（`name`、`time`、`value`、`factory`、`equipment`、`location`）に合わせます。ファイルの1行目に`name,time,value`のような列名がある場合は、`--header columns`を指定すると、その列名に従ってフィールドを対応付けます。`machbase-neo shell`は現在のディレクトリを`/work`にマウントするため、ファイルがあるディレクトリでコマンドを実行します。次の例では、時刻を秒単位のUnixエポック時刻として指定します。
...
日時文字列を使用する場合は、`timeformat`を明示します。JSONペイロードでは、HTTP書き込みAPIの`data.columns`と`data.rows`でデータを指定します。
...
### ロールアップと統計 {#rollup-and-statistics}

- `WITH ROLLUP`を指定すると、秒・分・時間単位の自動集計テーブルが作成され、時間区間ごとの分析を高速化できます。
- `TAG_STAT_ENABLE=1`の場合、`v$<table>_stat`ビューで、タグごとの件数、最小・最大値、最新時刻などの統計を確認できます。

### ビューの構成例 {#view-configuration-example}
...
- **パーティション数の選択**：テーブル作成時に、高性能サーバーでは大きい`TAG_PARTITION_COUNT`、エッジ機器では小さい値を選択して、リソース使用量を調整します。

タグテーブルは、簡単なスキーマで大量の時系列データを取り込み、分析するために設計されています。ロールアップ、統計、保持ポリシーと組み合わせることで、完成度の高い時系列データプラットフォームを構成できます。
