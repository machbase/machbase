...
## MySQLブリッジの登録 {#mysql-브리지-등록}

MySQLデータベースに接続するブリッジを登録します。
...
TIMESTAMP型の列を正しく処理するには、`parseTime=true`オプションを必ず指定してください。
{{< /callout >}}
...
machbase-neoシェルで以下のコマンドを実行し、`my`ブリッジを介して`my_example`テーブルを作成します。
...
## TQLによるMySQLへの書き込み {#mysql에-tql로-쓰기}
...
## TQLによるMySQLからの読み取り {#mysql에서-tql로-읽기}
