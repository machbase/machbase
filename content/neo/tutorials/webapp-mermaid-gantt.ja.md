---
toc: true
title: mermaid.jsのガントチャート
type: docs
weight: 33
---

この例は、mermaid.jsライブラリを使ってガントチャートを表示する方法を示します。

{{< figure src="/neo/tutorials/img/gantt-webapp-1.jpg" width="600px" >}}

- 7行目：`class="mermaid"`を指定した`<div>`を描画領域として使用します。
- 28行目：各状態の最初と最後のレコードを抽出する`FILTER_CHANGED()` {{< neo_since ver="8.0.15" />}}を使用します。
- 46〜48行目：mermaidスクリプトを追加します。
- 56行目：`startOnLoad:false`オプションでmermaid.jsを読み込みます。
