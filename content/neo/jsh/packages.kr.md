...
`pkg install -g`, `pkg uninstall -g`에서는 루트가 항상 `/work`입니다.
...
- 재현 가능한 설치를 위해 `pkg`는 `package.json` 옆에 `package-lock.json`도 기록합니다.
- `bin`은 `node_modules` 아래 각 설치 패키지의 자체 `package.json`에서 읽어 오며, `pkg`는 그것을 바탕으로 `node_modules/.bin` wrapper를 생성합니다.

예를 들어 `pkg install -g github.com/acme/demo`를 실행하면 갱신되는 프로젝트 manifest는 `/work/package.json`이고, 설치된 패키지 manifest는 `/work/node_modules/github.com/acme/demo/package.json`입니다.
...
- `-g, --global` `--dir`를 무시하고 `/work` 및 `/work/node_modules`에 설치합니다.
- `-h, --help` 도움말을 표시합니다.
...
- `-C, --dir <dir>` 현재 작업 디렉터리 대신 지정한 프로젝트 디렉터리를 사용합니다.
- `-g, --global` `--dir`를 무시하고 `/work`에서 패키지를 제거합니다.
- `-h, --help` 도움말을 표시합니다.
...
`pkg install -g <name>`은 `/work`를 패키지 루트로 사용하고 `/work/node_modules`에 설치합니다.
