...
複数のロールアップが一致する場合、指定したテーブルを強制します。ヒントは常に自動選択より優先されます。

```sql
SELECT /*+ ROLLUP_TABLE(rollup_table_name) */ ...
```

- ヒントがない場合、間隔、値列、JSON パスが同じ候補のうち、フィルターなしのロールアップを優先します。
- フィルター付きロールアップを選択する場合はヒントを使用します。
- `FIRST()`/`LAST()` では `EXTENSION` ロールアップを指定してください。
