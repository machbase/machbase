...
라이선스 파일이 설치되지 않았거나 비정상적인 파일을 사용한 경우 `machadmin -f` 또는 `V$LICENSE_INFO`로 라이선스 상태를 확인합니다. Standard 8.5에서는 동일한 Version 3 필드를 사용하며, 위반 상태와 메시지는 `V$LICENSE_INFO`의 `VIOLATE_STATUS`, `VIOLATE_MSG` 컬럼으로 확인합니다.
