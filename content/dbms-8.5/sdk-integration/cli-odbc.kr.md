...
CLI는 데이터베이스에 SQL을 어떻게 전달하고, 결과 값을 어떻게 받아 분석해야 하는지에 대한 함수와 명세를 정의합니다. CLI는 1990년대 초에 C와 COBOL 언어만을 위해 개발되었으며, 그 명세가 현재까지 유지되고 있습니다.

현재까지 가장 널리 알려진 표준 인터페이스는 ODBC(Open Database Connectivity)로, 클라이언트 프로그램이 데이터베이스의 종류와 무관하게 데이터베이스에 접속하는 방법을 제공합니다. 현재 최신 ODBC API 버전은 3.52이며 ISO와 X/Open 표준에 정의되어 있습니다.


## 표준 CLI 함수

표준 함수의 사용법은 다음 링크를 참조합니다.
...
CLI로 접속하려면 연결 스트링을 만들어야 하며, 각 항목은 다음과 같습니다.
...
|   SHOW_HIDDEN_COLS  | 숨겨진 컬럼(`_arrival_time`)을 `select *` 수행 시 보여줄 것인지 결정합니다.<br> 0일 경우에는 보이지 않으며, 1일 경우에 해당 컬럼의 정보가 출력됩니다.  |
|  CONNECTION_TIMEOUT | 최초 연결 시에 얼마나 대기할 것인지 설정합니다.<br> 디폴트로는 30초가 설정되어 있습니다.<br> 최초 연결 시 서버의 응답이 30초보다 느려질 수 있다면 이 값을 더 크게 설정해야 합니다.<br> CONNECTION_TIMEOUT에서 0 값은 Timeout에 제한이 없음을 의미하며 연결이 실패할 때에도 무한정으로 대기하므로 되도록이면 사용하지 않는 편이 좋습니다.  |
...
CLI 확장 함수는 Machbase 서버에 데이터를 초고속으로 입력하기 위해 제공되는 Append 프로토콜을 구현하는 함수입니다.

이 함수는 크게 채널 오픈, 채널에 대한 데이터 입력, 채널 플러시, 채널 클로즈의 4가지로 구성되어 있습니다.

### Append 프로토콜의 이해

Machbase에서 제공하는 Append 프로토콜은 비동기 방식으로 동작합니다. 비동기란 클라이언트가 서버에 요청한 작업에 대한 응답이 요청과 완전히 동기화되지 않고, 임의의 이벤트가 발생하는 순간에 전달되는 것을 의미합니다. 즉, 클라이언트가 Append를 수행했더라도 그 수행 결과를 바로 얻거나 확인할 수 없으며, 서버에서 준비가 되는 임의의 시점에 확인할 수 있습니다. 이런 이유로 Append 프로토콜을 활용해 응용 프로그램을 개발하는 개발자는 다음과 같은 내부 동작을 이해해야 합니다. 이후의 설명은 클라이언트가 서버에서 발생하는 비동기 에러를 언제, 어떻게 검출하여 사용자에게 돌려주는지에 대한 것입니다.

### Append 데이터의 전송

SQLExecute()나 SQLExecDirect()와 같은 일반적인 호출에서 Machbase는 즉시 그 결과를 클라이언트에게 돌려주는 동기 방식을 사용합니다. 그러나 SQLAppendDataV2()는 사용자 데이터가 입력된 직후에 요청을 보내지 않습니다. 대신 클라이언트 통신 버퍼가 모두 찰 때까지 기다렸다가, 버퍼가 차면 데이터를 한꺼번에 서버로 전송합니다. 이렇게 설계한 이유는 Append를 사용하는 클라이언트의 입력 데이터가 초당 수만에서 수십만 레코드라고 가정했기 때문이며, 고속 데이터 전송을 위해 버퍼링 방식을 활용합니다. 따라서 사용자가 임의의 시점에 버퍼의 내용을 전송하려면 SQLAppendFlush() 함수를 호출하여 명시적으로 데이터를 입력할 수 있습니다.

### Append 데이터의 에러 확인

앞에서 언급한 바와 같이 Append 프로토콜은 버퍼링되어 비동기로 동작합니다. 특히 서버에서 에러가 발생하지 않으면 아무런 응답을 받지 않고, 에러가 발생했을 때만 에러를 검출하는 방식이므로 에러가 언제, 어떻게 검출되는지 이해하는 것이 매우 중요합니다. 또한 에러를 검출하는 비용이 상대적으로 매우 커서 레코드를 입력할 때마다 검사하는 것은 매우 비효율적이므로, 현재 Machbase는 다음과 같은 경우에만 명시적으로 에러를 검출합니다. 에러가 검출되면 사용자가 설정한 에러 콜백 함수를 매번 호출합니다.

1. 전송 버퍼가 모두 차서 서버에 명시적으로 데이터를 전송한 이후 검사
2. SQLAppendFlush() 내부에서 서버에 명시적으로 데이터를 전송한 이후 검사
3. SQLAppendClose() 내부에서 종료 직전에 검사

즉, 기본적으로 위의 3가지 경우에만 에러를 검출하여 I/O 발생을 최소화하도록 설계되었습니다.

### 서버 에러 검사를 위한 부가 옵션

성능을 최대한 확보하기 위해 기본으로 설정된 에러 검출 방식은, 사용자가 원하면 더 자주 검사하도록 바꿀 수 있습니다. SQLAppendOpen() 함수의 마지막 인자인 aErrorCheckCount를 조절하면 됩니다. 이 값이 0이면 별도의 확인 동작 없이 기본 방식으로 동작합니다. 이 값이 0보다 크면 SQLAppendData()를 그 횟수만큼 호출할 때마다 명시적으로 에러를 검사합니다. 예를 들어 이 값이 10이면 Append를 10번 수행할 때마다 에러를 검사하는 비용을 지불합니다. 따라서 이 값이 작으면 에러 검출에 시스템 리소스를 많이 사용하므로 적절한 값으로 조절해야 합니다.

### 서버 에러 발생시 Trace 로그 남기기

에러가 발생한 Append 데이터에 대해 별도로 Trace 로그를 남기려면 서버 프로퍼티 DUMP_APPEND_ERROR를 1로 설정합니다. 이렇게 설정하면 에러를 발생시킨 레코드의 명세가 mach.trc 파일에 기록됩니다. 단, 에러가 과도하게 발생하면 시스템 리소스 사용량이 급격히 늘어나 Machbase의 전체 성능이 떨어질 수 있으므로 주의하여 사용해야 합니다.
...
이 함수는 대상 테이블에 대한 채널을 오픈합니다. 채널을 닫기 전까지는 계속 열린 상태가 유지됩니다.

하나의 연결에 최대 1024개의 Statement를 설정할 수 있습니다. 각 Statement마다 SQLAppendOpen을 사용하면 됩니다.

1. aStatementHandle: Append를 수행할 Statement의 핸들을 나타냅니다.
2. aTableName: Append를 수행할 대상 테이블의 이름을 나타냅니다.
3. aErrorCheckCount: 몇 건의 데이터가 입력될 때마다 서버의 에러를 검사할 것인지 결정합니다. 이 값이 0이면 별도로 에러를 검사하지 않습니다.
...
이 함수는 해당 채널에 데이터를 입력하는 함수입니다.

* aData는 입력될 데이터의 포인터를 담고 있는 배열입니다. 배열의 개수는 Open 시에 지정한 테이블이 보유하고 있는 컬럼의 개수와 일치해야 합니다.
* 리턴값은 SQL_SUCCESS, SQL_SUCCESS_WITH_INFO, SQL_ERROR가 가능합니다. 특히 SQL_SUCCESS_WITH_INFO가 반환되었다면 입력된 특정 컬럼의 길이가 길어 잘리는 등의 오류가 있을 수 있으므로 결과를 다시 확인해야 합니다.

**데이터 타입에 따른 설정**

숫자형 및 문자형

* float, double, short, int, long long, char * 과 같은 타입은 해당 값에 대한 포인터를 설정하는 것만으로 동작합니다.

주소형

* IPv4는 5바이트 무부호 문자(unsigned char)의 배열로 넘깁니다.
* 첫 번째 바이트는 4로 설정하고, 이후의 4바이트는 연속되는 주소값으로 설정합니다.
* 예를 들어 127.0.0.1은 5바이트 배열 **0x04, 0x7f, 0x00, 0x00, 0x01**의 순으로 들어갑니다.
...
* Machbase는 내부적으로 나노초 단위의 시간 해상도를 가지므로 클라이언트에서 시간을 설정할 때는 변환 과정을 거쳐야 하며, 이 값은 64비트 부호 없는 정수로 표현됩니다. 적절히 변환하려면 유닉스 라이브러리 함수인 mktime으로 초 단위로 변환한 뒤 초 미만의 값을 더해야 합니다.
* ※ Machbase의 시간 = (1970년 1월 1일 이후로부터의 총 시간 (초)) * 1,000,000,000 + milli-second * 1,000,000 + micro-second * 1000 + nano-second;
...
이 함수는 해당 채널에 데이터를 입력하는 함수이며, DB에 저장되는 `_arrival_time` 값을 현재 시간이 아닌 특정 시간의 값으로 설정할 수 있습니다.

예를 들면, 1개월 전 로그 파일에 있는 날짜를 그 당시의 날짜로 입력하고자 할 때 사용합니다.

* aTime은 `_arrival_time`으로 설정될 time 값입니다.
* aData는 입력될 데이터의 포인터를 담고 있는 배열입니다.
* 배열의 개수는 Open 시에 지정한 테이블이 보유하고 있는 컬럼의 개수와 일치해야 합니다.
...
이 함수는 Machbase 2.0부터 새로 도입된 Append 함수로, 기존 함수에서 불편했던 입력 방식을 대폭 개선한 함수입니다.

특히 2.0에서 도입된 TEXT와 BINARY 타입은 SQLAppendDataV2() 함수로만 입력할 수 있습니다.

* 각 타입에 맞는 NULL 입력 가능
* VARCHAR 입력 시 스트링 길이 입력 가능
* IPv4, IPv6 입력 시 바이너리 및 스트링 형태의 데이터 입력 가능
* TEXT, BINARY 타입에 대한 데이터 길이 지정 가능

함수 인자는 다음과 같이 구성됩니다.

* aData는 SQL_APPEND_PARAM이라는 인자 배열을 가리키는 포인터입니다. 이 배열의 개수는 Open 시에 지정한 테이블이 보유하고 있는 컬럼의 개수와 일치해야 합니다.
* 리턴값은 SQL_SUCCESS, SQL_SUCCESS_WITH_INFO, SQL_ERROR가 가능합니다. 특히 SQL_SUCCESS_WITH_INFO가 반환되었다면 입력된 특정 컬럼의 길이가 길어 잘리는 등의 오류가 있을 수 있으므로 결과를 다시 확인해야 합니다.

아래는 V2에서 사용하는 SQL_APPEND_PARAM의 정의이며, 이 내용은 machbase_sqlcli.h에 포함되어 있습니다.
...
위에서 볼 수 있듯이 내부적으로 machbaseAppendParam이라는 공용체가 하나의 인자를 담는 구조입니다. 각 데이터 타입에 대해 데이터 및 스트링의 길이와 값을 명시적으로 입력할 수 있습니다. 실제 사용 예는 다음과 같습니다.

**고정 길이 숫자형 타입의 입력**

고정 길이 숫자형 타입은 short, ushort, integer, uinteger, long, ulong, float, double을 말합니다. 이 타입은 SQL_APPEND_PARAM의 구조체 멤버에 직접 값을 대입하여 입력할 수 있습니다.
...
아래는 DATETIME형의 데이터를 입력하는 예입니다. 편의를 위해 몇 가지 매크로가 준비되어 있습니다.

SQL_APPEND_PARAM에서 mDateTime 멤버를 조작합니다. 아래의 매크로는 mDateTime 구조체의 64비트 정수값인 mTime을 설정하여 날짜를 지정합니다.
...
|        임의의 64비트 값       | 이 값이 실제 datetime으로 입력됩니다.<br> 이 값은 1970년 1월 1일 이후로 흐른 시간을 나노초 단위로 나타낸 정수값입니다. <br>예를 들어, 이 값이 10억 (1,000,000,000)이라면 1970년 1월 1일 0시 0분 1초를 나타냅니다.(GMT) |
...
아래는 IPv4와 IPv6 형의 데이터를 입력하는 예입니다. 이 역시 편의를 위해 몇 가지 매크로가 준비되어 있습니다. SQL_APPEND_PARAM에서 mLength 멤버를 조작합니다.
...
IP 타입을 문자열(STRING)로 입력하면 SQLAppendDataV2() 호출 이후에 mLength가 자료형에 맞게 4 또는 6으로 바뀝니다. 따라서 반복문에서 입력할 때는 매번 SQLAppendDataV2()를 호출하기 전에 mLength를 SQL_APPEND_IP_STRING으로 지정해 주어야 합니다.

**가변 데이터형(문자 및 이진 데이터) 입력**

가변 데이터형에는 VARCHAR, TEXT, BLOB, CLOB이 포함됩니다. 기존 함수에서는 VARCHAR만 지원되었고, 사용자가 스트링의 길이를 입력할 방법도 없었습니다. 그래서 매번 strlen() 함수로 길이를 얻어야 했지만, V2부터는 사용자가 가변 데이터형의 길이를 직접 지정할 수 있습니다. 따라서 사용자가 길이를 미리 알고 있다면 더 빠르게 데이터를 입력할 수 있습니다. 내부적으로 가변 데이터형은 하나의 구조체로 되어 있지만, 개발 편의를 위해 데이터 타입별로 멤버를 따로 만들어 두었습니다.
...
가변 데이터형을 입력할 때는 데이터의 길이를 mLength에 설정하고, 원시 데이터 포인터를 mData에 설정하면 됩니다. mLength가 정의된 스키마보다 크면 자동으로 잘려서 입력됩니다. 이때 SQLAppendDataV2() 함수는 SQL_SUCCESS_WITH_INFO를 리턴하고, 관련 경고 메시지를 내부 구조체에 채웁니다. 이 경고 메시지를 확인하려면 SQLError() 함수를 이용하면 됩니다.
...
다음은 각각의 경우에 대해 실제 값을 입력하는 예제입니다. 하나의 VARCHAR 컬럼이 존재한다고 가정합니다.
...
이 함수는 해당 채널에 데이터를 입력하는 함수이며, DB에 저장되는 `_arrival_time` 값을 현재 시간이 아닌 특정 시간의 값으로 설정할 수 있습니다. 예를 들면, 1개월 전 로그 파일에 있는 날짜를 그 당시의 날짜로 입력하고자 할 때 사용합니다.

* aTime은 `_arrival_time`으로 설정될 time 값입니다. 1970년 1월 1일 이후로부터 현재까지의 나노초 값을 입력해야 합니다. 또한 입력되는 값은 과거부터 현재 순으로 정렬되어 있어야 합니다.
* aData는 입력될 데이터의 포인터를 담고 있는 배열입니다. 배열의 개수는 Open 시에 지정한 테이블이 보유하고 있는 컬럼의 개수와 일치해야 합니다.
...
이 함수는 현재 채널 버퍼에 쌓여 있는 데이터를 Machbase 서버로 즉시 전송합니다.
...
이 함수는 현재 열린 채널을 닫습니다. 채널이 열려 있지 않으면 에러가 발생합니다.

* aSuccessCount: Append에 성공한 레코드 개수 값을 가집니다.
* aFailureCount: Append에 실패한 레코드 개수 값을 가집니다.
...
이 함수는 SQLAppendOpen()이 성공한 다음 Append 시 에러가 발생했을 때 호출되는 콜백 함수를 설정합니다. 이 함수를 설정하지 않으면 서버에서 에러가 발생하더라도 클라이언트에서는 무시합니다.

* aStmtHandle: 에러를 확인할 Statement를 지정합니다.
* aFunc: Append 실패 시 호출할 함수 포인터를 지정합니다.
...
* aStatementHandle: 에러를 발생시킨 Statement 핸들
* aErrorCode: 에러의 원인이 된 32비트 에러 코드
* aErrorMessage: 해당 에러 코드에 대한 문자열
...
Append로 입력된 데이터는 통신 버퍼에 기록되어 전송 대기 상태로 있다가, 사용자가 SQLAppendFlush 함수를 호출하거나 통신 버퍼가 가득 차면 서버로 전송됩니다. 버퍼가 가득 차지 않아도 일정 주기로 Append 데이터를 서버에 전송하게 하려면 이 함수를 이용하면 됩니다. 이 함수는 100ms마다 마지막으로 전송한 시간과 현재 시간의 차이를 계산하여, 지정된 시간(설정하지 않은 경우에는 1초)이 지나면 통신 버퍼의 내용을 서버에 전달합니다.

매개변수는 다음과 같습니다.

* hdbc: DB의 connection handle입니다.
* option: 0이면 auto flush를 끄고, 0이 아닌 값이면 auto flush를 켭니다.

연결되지 않은 hdbc에 대해 실행하면 오류로 처리됩니다.
...
SQLSetConnectAppendFlush로 시간 단위 flush 기능을 켠 상태에서, 특정 statement에 대해 자동 flush를 끄거나 flush 주기를 조정하려면 이 함수를 사용합니다.

매개변수는 다음과 같습니다.

* hstmt: flush 주기를 조정하려는 statement handle입니다.
* fValue: 조정하려는 flush 주기 값입니다. **0이면 flush를 하지 않으며 단위는 ms입니다**. 100ms마다 flush 여부를 결정하는 스레드가 실행되므로 100의 배수로 설정합니다. 원하는 시점에 정확히 자동 flush가 실행되지는 않습니다. **1000이 기본 값입니다**.

시간 기반 flush가 실행 중이지 않아도 이 함수의 실행은 성공합니다.

**Error 확인 및 설명**

Append 관련 함수를 사용할 때 에러를 확인하는 방법과 코드에 대한 설명입니다. CLI 함수의 return 값이 SQL_SUCCESS가 아니면 아래 코드로 에러 메시지를 확인할 수 있습니다.
...
Append 관련 함수에서 리턴되는 에러 메시지는 아래와 같습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>column meta 정보 구조가 잘못되었습니다.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>return 값에 오류가 있습니다.</td>
    </tr>
    <tr>
      <td rowspan="3">SQLAppendData</td>
      <td>statement is not opened.</td>
      <td>AppendOpen을 하지 않고 AppendData를 호출했습니다.</td>
    </tr>
    <tr>
      <td>column() truncated :</td>
      <td>varchar 타입 컬럼에 지정된 사이즈보다 큰 데이터를 입력하는 경우 발생합니다.</td>
    </tr>
    <tr>
      <td>Failed to add binary.</td>
      <td>통신 버퍼에 쓰기 오류가 발생합니다.</td>
    </tr>
    <tr>
      <td rowspan="5">SQLAppendClose</td>
      <td>statement is not opened.</td>
      <td>AppendOpen 상태가 아닙니다.</td>
    </tr>
    <tr>
      <td>Failed to close stream protocol.</td>
      <td>스트림 프로토콜 종료에 실패했습니다.</td>
    ...
    <tr>
      ...
      <td>통신 버퍼 읽기 결과가 APPEND_DATA_PROTOCOL 값이 아닙니다.</td>
    </tr>
    <tr>
      <td rowspan="8">SQLAppendDataV2</td>
      <td>Invalid date format or date string.</td>
      <td>날짜/시간 형식이 잘못된 경우 발생합니다.</td>
    </tr>
    <tr>
      <td>statement is not opened.</td>
      <td>AppendOpen 상태가 아닙니다.</td>
    ...
    <tr>
      <td>column() truncated :</td>
      <td>binary 타입 컬럼에 지정된 사이즈보다 큰 데이터를 입력하는 경우 발생합니다.</td>
    </tr>
    <tr>
      <td>column() truncated :</td>
      <td>varchar 또는 text 타입 컬럼에 지정된 사이즈보다 큰 데이터를 입력하는 경우 발생합니다.</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <td>IPv4, IPv6 타입 구조체의 mLength 값이 잘못 지정되었습니다.</td>
    </tr>
    <tr>
      <td>IP string is invalid.</td>
      <td>IPv4 또는 IPv6 형식이 아닙니다.</td>
    </tr>
    <tr>
      <td>Unknown data type has been specified.</td>
      <td>Machbase에서 사용하는 데이터 타입이 아닙니다.</td>
    </tr>
  </tbody>
</table>
...
Machbase에 대량의 데이터를 빠르게 입력하는 SQLAppend 함수는 로그/태그 테이블에 입력할 때만 사용할 수 있으며, Lookup 테이블이나 Volatile 테이블에 대량 업데이트를 수행할 때는 사용할 수 없습니다.

이를 위해 Machbase 5.5 이후 버전에서는 열 형식 매개변수 바인딩을 지원합니다. (행 형식 매개변수 바인딩은 아직 지원되지 않습니다.)

SQLSetStmtAttr() 함수의 인자 Attribute에 SQL_ATTR_PARAM_BIND_TYPE을, 인자 param에 SQL_PARAM_BIND_BY_COLUMN을 설정합니다. 바인드할 각 컬럼에 대해 매개변수를 배열로 설정하고, 지시자 변수도 배열로 설정합니다. 그런 다음 이 매개변수를 전달하여 SQLBindParameter()를 호출합니다.
...
Machbase는 기본적으로 UTF-8 방식으로 문자열 데이터를 저장합니다.

UTF-8 이외의 방식으로 문자열을 입출력하는 Windows에서는 ODBC가 아래와 같이 변환합니다.

|    OS   | Unicode/Non-Unicode |   문자열 변환  |                              Note                              |
|:-------:|:-------------------:|:--------------:|:--------------------------------------------------------------:|
| Windows | Unicode (UTF-16)    | UTF-16 ⟷ UTF-8 | N/A                                                            |
| Windows | Non-Unicode (MBCS)  | MBCS ⟷ UTF-8   | Windows 설정에서 Non-Unicode 애플리케이션의 기본 문자열을 사용함 |
| Linux   | UTF-8               | N/A            | UTF-8만 지원됨                                                |
