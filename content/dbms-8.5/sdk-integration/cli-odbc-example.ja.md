...
### CLI のインストール確認 {#check-cli-installation}

インストール先の include と lib に次のファイルがあれば、開発環境が整っています。
...
Machbase パッケージには、次の場所にサンプルプログラムが含まれます。
...
> サンプルの Makefile を変更して、用途に合ったアプリケーションを作成できます。

## サンプルプログラム {#sample-program}

### 接続の例 {#connection-example}

CLI を使用して接続するプログラムを示します。
ファイル名は sample1_connect.c です。
MACHBASE_PORT_NO は、$MACHBASE_HOME/conf/machbase.conf の PORT_NO と一致させてください。
...
sample1_connect.c を Makefile に登録し、コンパイルして実行すると、次のように表示されます。
...
sample2_insert.c を Makefile に登録し、コンパイルして実行すると、次のように表示されます。
...
### プリペアードステートメントの実行例 {#prepare-execute-example}

データをバインドして INSERT する例を示します。
Machbase では、データをバインドして値を入力できます。データ型を明示し、長い文字列では長さも指定してください。
各型のデータをバインドする例です。
ファイル名は sample3_prepare.c です。
...
sample3_prepare.c を Makefile に登録し、コンパイルして実行すると、次のように表示されます。
...
Machbase は、ファイルなどから大量のデータを高速入力する Append プロトコルを提供します。その使用例を示します。

まず、各データ型で APPEND を使用する方法を確認します。型ごとに設定方法が異なるため、各方式を理解すると効率よく実装できます。下のコードに、各方式をまとめています。

ファイル名は sample4_append1.c です。
...
void appendData()
{
    ...
    *(in_addr_t *)(sParam[7].mIP.mAddr) = inet_addr("192.168.0.1");
    ...
    SQLAppendDataV2(gStmt, sParam);
    sParam[8].mIP.mLength = SQL_APPEND_IP_NULL; /* 復旧 */
...
sample4_append1.c を Makefile に登録し、コンパイルして実行すると、次のように表示されます。
...
次に、ファイルを使った高速 APPEND を示します。大量のログやパケットなどの入力に適しています。ファイル名は sample4_append2.c です。

入力するデータを、事前に data.txt に保存してください。

```bash
./make_data
```

付属の make_data.c を変更すると、用途に合った data.txt を作成できます。
...
### テーブルの列情報の取得例 {#acquiring-table-column-information-example}

列情報を取得する方法のうち、SQLDescribeCol と SQLColumns を使用する方法を説明します。

#### SQLDescribeCol {#sqldescribecol}

SQLDescribeColは、テーブルの列番号、名前、バッファーサイズ、長さ、型などを取得する関数です。これらの情報を使って、データベースから必要な列情報を取得できます。

ファイル名は sample5_describe.c です。
...
SQLColumns は、テーブルに存在する列の情報を取得する関数です。Machbase でもサポートしており、各列の情報を確認できます。

ファイル名は sample6_columns.c です。
...
### マルチスレッド APPEND の例 {#multi-thread-append-example}

1 つのプログラム内の複数スレッドで、複数のテーブルに APPEND する例です。

ファイル名は sample8_multi_session_multi_table.c です。
