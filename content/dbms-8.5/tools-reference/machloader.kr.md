...
* 입력 대상 테이블의 데이터를 삭제한 후 입력하려면 `-m replace` 옵션을 사용합니다.
...
|-b, --bad=BAD_FILE|-i 옵션 실행 시 입력 오류가 발생한 데이터와 오류 설명을 기록할 파일 이름 지정|
|-m, --mode=MODE|-i 옵션 실행 시 가져오기 방법 지정. append 또는 replace를 사용할 수 있습니다. append는 기존 데이터 뒤에 데이터를 입력하고, replace는 기존 데이터를 삭제한 후 데이터를 입력합니다.|
...
가져오기(`-i`) 시 `replace`와 `append` 두 모드를 사용할 수 있습니다. 기본값은 `append`입니다. `replace` 모드는 기존 데이터를 삭제하므로 주의해서 사용해야 합니다.
...
서버 IP, 포트, 사용자, 비밀번호를 별도로 지정합니다.

옵션:

- `-s`: 서버 IP 주소를 지정합니다(기본값: 127.0.0.1).
- `-P`: 서버 포트 번호를 지정합니다(기본값: 5656).
- `-u`: 접속 사용자 이름을 지정합니다(기본값: SYS).
- `-p`: 접속 사용자 비밀번호를 지정합니다(기본값: MANAGER).
...
machloader 실행 로그 파일과 bad-data 파일을 생성합니다.

옵션:

- `-b`: 가져오기에 실패한 행을 기록할 bad-data 파일 이름을 지정합니다.
- `-l`: 가져오기에 실패한 행과 오류 메시지를 기록할 실행 로그 파일 이름을 지정합니다.
...
machloader 스키마 파일을 생성할 수 있습니다. 스키마 파일을 사용하면 데이터 타입 형식을 변경하거나 테이블의 컬럼 수와 데이터 파일의 필드 수가 다른 경우에도 가져오기/내보내기를 수행할 수 있습니다.

옵션:

- `-c`: 스키마 파일을 생성합니다.
- `-t`: 테이블 이름을 지정합니다.
- `-f`: 생성할 스키마 파일 이름을 지정합니다.
...
모든 datetime 컬럼에 설정하는 구문:

```
DATEFORMAT <dateformat>
```

개별 datetime 컬럼에 설정하는 구문:
...
CSV 파일의 특정 필드를 입력하지 않으려면 fmt 파일에서 `IGNORE` 옵션을 설정할 수 있습니다.
`ignoretest.csv` 파일에는 필드가 3개 있습니다. 마지막 필드가 필요 없다면 fmt 파일에서 해당 컬럼에 `IGNORE`를 지정합니다.
...
테이블의 컬럼 수가 데이터 파일의 필드 수보다 많으면, 스키마 파일에 지정한 컬럼에만 데이터가 입력되고 나머지 컬럼에는 `NULL`이 입력됩니다.

## 컬럼 수가 필드 수보다 적은 경우

테이블의 컬럼 수가 데이터 파일의 필드 수보다 적으면, 테이블에 없는 필드는 `IGNORE` 옵션으로 제외해야 합니다.
