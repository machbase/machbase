...
| 도구 | 용도 | 일반적인 사용 |
|------|---------|------------|
| machadmin | 서버 관리 | 시작/중지, 복구 |
| machsql | SQL 클라이언트 | 쿼리, 스크립트 |
| machloader | 데이터 가져오기 | CSV 로딩 |
| csvimport/csvexport | CSV 래퍼 | 간단한 가져오기/내보내기 |
...
- `PROCESS_MAX_SIZE` - 서버 프로세스의 최대 메모리 크기
- `DISK_COLUMNAR_TABLE_CHECKPOINT_INTERVAL_SEC` - 테이블 체크포인트 간격
- `DISK_COLUMNAR_INDEX_CHECKPOINT_INTERVAL_SEC` - 인덱스 체크포인트 간격
...
4. **유지보수 입력값 확인** - 복구/추출 작업 전에 백업 이미지 경로를 확인하세요
...
자세한 도구 문서는 이 섹션의 각 도구 페이지를 참조하세요.
