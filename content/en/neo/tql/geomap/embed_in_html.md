...
Call *.tql* script file with a custom HTTP header `X-Chart-Output: json` 
...
The `X-Chart-Output: json` header is actually equivalent to the `GEOMAP()` SINK with `geoMapJson(true)` option like `GEOMAP( geoMapJson(true)))`.

When the reponse of `/db/tql/basic_map.tql` is JSON, it contains required addresses of the result javascript.

```json
{
    "ID":"MTcwMzE3NjYwMjA0Nzg1NjY0",
    "style": {
        "width": "600px",
        "height": "600px",
        "grayscale": 0
...
- `ID` random generated ID of a map, a client can set a specific ID with `mapId()` option.
...
- Line 17, The HTTP header `X-Chart-Output: json`.
