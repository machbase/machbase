...
{{< callout emoji="📌" >}}
The example codes that uses database table, assumes that you have the example table and some data like below.
...
`strTrimPrefix` returns str without the provided leading prefix string. If s doesn't start with prefix, str is returned unchanged.
...
*Syntax*: `parseBoolean( str )` {{< neo_since ver="8.0.7" />}}
