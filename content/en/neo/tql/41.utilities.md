...
*Syntax*: `diff( value )` {{< neo_since ver="8.0.8" />}}

- `value` *float*

`diff()` returns the difference between current and previous values (*current - previous*). 

### nonNegativeDiff()

*Syntax*: `nonNegativeDiff( value )` {{< neo_since ver="8.0.8" />}}

- `value` *float*

`nonNegativeDiff()` returns the difference between current and previous values (*current - previous*).
If the difference is less than zero it return zero instead of minus value.

### movavg()

*Syntax*: `movavg(value, lag)`  {{< neo_since ver="8.0.7" />}}

- `value` *float*
- `lag` *integer* specifies how many records it accumulates.

`movavg` returns a moving average of values by given lag count.
If input values are not accumulated enough to the `lag`, it returns `NULL` instead.
If all incoming values are `NULL` (or not a number) for the lastest `lag` count, it returns `NULL`.
If some accumulated values are `NULL` (or not a number), it makes average value from only valid values by excluding the `NULL`s.

**Limitation**: a single statement can not contains multiple `movavg`. One `movavg()` call per a statement.
For example, `MAPVALUE(1, movavg(value(0), 10))` is valid, but `MAPVALUE(1, movavg(value(2), 10) + movavg(value(3), 20))` returns an unpredictable result. 
