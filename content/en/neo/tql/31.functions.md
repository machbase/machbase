...
- `fields` *string* {{< neo_since ver="8.0.7" />}} Aggregation functions for each columns, if no column is specified `GROUPBYKEY` make new array of the raw records by default.
...
If `fields` exists `GROUPBYKEY` yields the statistic values of the field implies as below.
