...
- `fields` *string* specifies each field's aggration function and indicates which column is the time. It should be one of `time`, `avg`, `sum`, `first`, `last`, `min`, `max`, `rss`.
