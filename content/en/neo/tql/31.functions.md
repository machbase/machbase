...
CHART_LINE3D(
  lineWidth(4), gridSize(100, 30, 100), visualMap(-0.12, 0.12)
)
```
{{< /tab >}}
{{< tab >}}
...
- How to notify an event to the specific HTTP endpoint.
...
- How to forward the current records to the specific HTTP endpoint in CSV which is default format of `doHttp`.
...
- How to send a custom JSON message to the specific HTTP endpoint.
