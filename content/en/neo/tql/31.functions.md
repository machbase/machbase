...
`THROTTLE` relays a record to next flow with delay to fit to the specified *tps*.

`THROTTLE()` makes data frow which has a certain period from stored data (e.g a CSV file).
It is useful to *simulate* a sensor device that sends a measurement by periods.
...
- At console log, each log time of each "tick" message has *200ms.* difference (5 per a second).
```
