...
*Syntax*: `GROUPBYKEY( [lazy(boolean)] [, fields] )`
...
*Syntax*: `GROUP( [lazy(boolean),] by [, aggregates...] )` {{< neo_since ver="8.0.7" />}}
...
- `aggregates` *array of aggregate* Aggregation functions

If no aggregate is specified `GROUP` make new array of the raw records for a each group by default.
...
If `aggregates` exists, `GROUP` yields the result value of the aggregate function as below.
...
| `mode`                | The most common value in the dataset Strict float64 equality is used when comparing values, so users should take caution. If several values are the mode, any of them may be returned. |
