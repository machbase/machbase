...
| method                |  description|
|:--------------------- | :----------------------------------------------------------------|
| `ctx.key()`           | key returns the key of the current record                        |
| `ctx.value(idx)`      | value returns the value of `idx`th value of the current record   |
| `ctx.drop()`          | drop discards the current record                                 |
| `ctx.yield(values...)`| yield produce a new record from the given values... arguments    |
| `ctx.yieldKey(key, values...)`| yield produce a new record from the given key and values... |
| `ctx.param(name)`     | param returns the request parameter.                             |
| `ctx.bridge(name)`    | returns bridge instance of the given name                        |
| `ctx.println(args...)`| print out log message                                            |
