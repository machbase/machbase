...
`diff()` replaces the value of the element at the given index with difference between current and previous values (*current - previous*). 
...
*Syntax*: `nonNegativeDiff( idx, value [, newName]  )` {{< neo_since ver="8.0.8" />}}
...
*Syntax*: `movavg(idx, value, lag [, newName] )`  {{< neo_since ver="8.0.8" />}}
