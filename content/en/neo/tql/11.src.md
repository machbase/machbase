...
- `src` data source, it can be one of `payload()`, `file()` and `string text`
- `separator(char)` *optional* default is `"\n"`
...
- `STRING('A,B,C', delimiter(","))` yields 3 records `["A"]`, `["B"]` and `["C"]`
...
```js
STRING( file(`http://example.com/data/words.txt`) )
```
