...
MySQL 데이터베이스에 연결할 브리지를 등록합니다.

연결 문자열은 MySQL 규격을 따릅니다.
...
TIMESTAMP 컬럼을 정확히 처리하려면 `parseTime=true` 옵션을 반드시 지정해야 합니다.
{{< /callout >}}
...
machbase-neo 셸에서 아래 명령을 실행해 `my` 브리지를 통해 `my_example` 테이블을 생성합니다.
