---
title: 마크베이스 매뉴얼
weight: 30
---


## 개요

* [마크베이스 소개](./intro/)
* [마크베이스 특징](./intro/features/)
* [마크베이스 제품군 소개](./intro/edition/)

## 설치

* [패키지 개요](./install/package)
* [리눅스 설치](./install/linux)
* [Windows 설치](./install/windows)
* [라이선스 설치](./install/license)
* [Cluster Edition 설치](./install/cluster)
* [Cluster Edition 업그레이드 및 복구](./install/upgrade)

## 주요기능 / 테이블

* [태그 테이블 (Tag Table)](./feature-table/tag)
* [로그 테이블 (Log Table)](./feature-table/log)
* [휘발성 테이블 (Volatile Table)](./feature-table/volatile)
* [참조 테이블 (Lookup Table)](./feature-table/lookup)
* [스트림 (Stream)](./feature-table/stream)
* [백업 및 마운트](./feature-table/backup-mount)
* [데이터 자동 삭제](./feature-table/retention)

## 설정 / 모니터링

* [Meta Table](./config-monitor/meta-table)
* [Virtual Table](./config-monitor/virtual-table)
* [Property](./config-monitor/property)
* [Property (Cluster)](./config-monitor/property-cl)
* [Timezone](./config-monitor/timezone)

## SQL Reference

* [자료형](./sql-ref/datatypes)
* [DDL](./sql-ref/ddl)
* [DML](./sql-ref/dml)
* [SELECT](./sql-ref/select)
* [SELECT Hint](./sql-ref/select-hint)
* [사용자 관리](./sql-ref/user-manage)
* [지원 함수](./sql-ref/func)
* [시스템/세션 관리](./sql-ref/sys-session-manage)

## SDK

* [CLI/ODBC](./sdk/cli-odbc)
* [CLI/ODBC 예제](./sdk/cli-odbc-example)
* [JDBC](./sdk/jdbc)
* [Python](./sdk/python)
* [RESTful API](./sdk/restful_api)
* [.NET Connector](./sdk/dotnet)

## 도구

* [machadmin](./tools/machadmin)
* [machsql](./tools/machsql)
* [machloader](./tools/machloader)
* [csvimport/csvexport](./tools/csv)
* [machcoordinatoradmin](./tools/machcoordinatoradmin)
* [machdeployeradmin](./tools/machdeployeradmin)

## 자주 묻는 질문(FAQ)

* [쿼리 에러를 Property 수정하여 해결하는 방법](./faq/faq)

## Archive

* [7.5 Manual](./archive/M75-Kor.pdf)
* [7.0 Manual](./archive/M7-Kor.pdf)
* [6.7 Manual](./archive/MK67-Kor.pdf)
* [6.5 Manual](./archive/M65-Kor.pdf)
* [6.0 Manual](./archive/M6-Kor.pdf)
* [5.0 Manual](./archive/M5-Kor.pdf)
