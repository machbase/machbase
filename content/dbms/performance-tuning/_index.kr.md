...
Machbase DBMS에서 성능 문제는 대부분 하나의 공통된 원인에서 비롯됩니다. 데이터 특성에 맞지 않는 테이블 타입 선택, 인덱스 없는 대용량 컬럼 스캔, 배치 입력 대신 건별 INSERT, 과소 설정된 메모리 버퍼가 그것입니다. 9장은 이 문제들을 체계적으로 진단하고 해결하는 방법을 다섯 영역으로 나누어 설명합니다.

**가장 큰 성능 이득은 올바른 테이블 타입 선택에서 옵니다.** 센서 계측값을 LOG 테이블로 저장하거나, 이벤트 로그를 TAG 테이블로 저장하면 아무리 인덱스를 추가하고 프로퍼티를 조정해도 근본적인 한계를 넘을 수 없습니다. 쿼리 최적화나 캐시 튜닝보다 테이블 타입 재설계가 수십 배 더 큰 성능 개선을 가져오는 경우가 흔합니다.
