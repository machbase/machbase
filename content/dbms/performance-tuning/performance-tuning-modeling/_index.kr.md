...
toc: true
---
모델링 단계의 선택이 이후 모든 튜닝의 천장을 결정합니다. 올바른 테이블 타입과 컬럼 구조를 선택하면 인덱스·캐시·프로퍼티 조정 없이도 충분한 성능을 얻을 수 있고, 잘못된 모델에서 출발하면 아무리 정교한 튜닝을 적용해도 근본적인 한계를 넘기 어렵습니다. 여기서는 테이블 타입 선택과 스키마 설계 관점에서 성능에 직접 영향을 미치는 결정 사항을 다룹니다.

## 왜 모델링이 성능의 기초인가

다섯 가지 테이블 타입은 각각 다른 저장 구조와 인덱싱 전략을 가집니다. TAG 테이블은 태그
이름과 시간을 조합한 인덱스를 자동 관리하고, LOG 테이블은 컬럼형 저장과 `_ARRIVAL_TIME`
기반 범위 조회를 제공합니다.

센서 계측값을 LOG 테이블에 넣으면 센서별 조회마다 전체 컬럼 스캔이 발생하고, 이벤트 로그를 TAG 테이블에 넣으면 파티션 구조가 의미 없어지며 메타데이터 공간이 낭비됩니다. 이 불일치는 인덱스를 아무리 추가해도 해결되지 않습니다.
...
LOG 테이블에서 특정 센서의 최근 값을 반복 조회하면 스캔과 인덱스 유지 비용을 함께 고려해야
합니다. 태그별 시계열 조회가 주된 패턴이면 TAG 테이블로 전환하고 같은 쿼리로 성능을
비교합니다.
