...
다섯 가지 테이블 타입은 각각 다른 저장 구조와 인덱싱 전략을 가집니다. TAG 테이블은 태그 이름과 시간을 조합한 파티션 인덱스를 자동 관리하여 수천 개 센서의 시계열 데이터를 고속으로 저장·조회합니다. LOG 테이블은 컬럼형 저장과 `_ARRIVAL_TIME` 기반 범위 스캔에 최적화되어 이벤트·로그 데이터를 효율적으로 처리합니다.
