...
| 랜덤 읽기 | 빠름 | 매우 느림 |
| 비용 | 높음 | 낮음 |
| 권장 용도 | 데이터 파일, WAL/redo 로그 | 콜드 데이터 아카이브 |

**SSD를 반드시 사용해야 하는 경우**

- 초당 10만 건 이상 Append가 발생하는 환경
- 여러 테이블을 동시에 쓰고 읽는 혼합 워크로드
- Cluster Edition에서 Active 노드 역할을 담당하는 호스트

WAL 및 redo 로그 파일은 특히 순차 쓰기 성능이 중요합니다. 데이터 파일과 WAL 파일을 서로 다른 물리 디스크(또는 다른 NVMe 슬롯)에 배치하면 I/O 경합을 줄일 수 있습니다.
