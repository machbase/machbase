...
```
전체 물리 메모리
  ├── OS와 함께 실행하는 프로세스의 예약분
  ├── Result Cache와 PVO Cache 상한
  ├── 입력 버퍼와 인덱스 메모리
  └── 정렬·집계·세션 등 실행 중 변동하는 처리 공간
```

- Result Cache와 PVO Cache 상한은 OS 예약분, 입력 버퍼, 인덱스 빌드와 쿼리 처리 공간을
  제외한 메모리 예산 안에서 설정합니다.
...
| 워크로드 | 설정 기준 | 확인 항목 |
|---------|-----------|-----------|
| 대시보드용 소규모 집계 | 반복 조회 결과가 상한 안에 포함되도록 설정 | 적중률, 쿼리당 메모리 |
| ROLLUP 결과 조회 | 실제 반환 행 수보다 약간 큰 상한부터 검증 | 캐시 점유율, LRU 교체 횟수 |
| 원시 데이터 대량 조회 | 캐시 제외 또는 낮은 상한 검토 | 메모리 압박, 다른 쿼리의 적중률 |
...
- 캐시 상한의 합과 입력 버퍼 상한을 동시에 계산하여 물리 메모리를 초과하지 않도록 합니다.
