...
증분 백업을 활용하면 전체 백업의 빈도를 줄이면서 복구 지점을 촘촘하게 유지할 수 있습니다.
복구할 때는 체인 정보를 포함한 최종 증분 백업 경로를 `machadmin -r`에 지정합니다.
...
- 백업 명령 실행 시 지정한 디렉터리가 자동으로 생성됩니다. 디렉터리가 이미 존재하면 오류가 발생하므로 날짜 등을 포함한 고유한 이름을 사용하십시오.
...
- 백업 경로에 충분한 디스크 여유 공간이 있는지 사전에 확인하십시오.
...
- 백업 경로가 이미 존재하면 오류가 발생합니다. 고유한 디렉터리 이름을 지정하십시오.
...
- FROM과 TO 범위가 정확하게 지정되어야 원하는 데이터가 포함됩니다. 범위 경계값 포함 여부를 확인하십시오.
...
> 복원을 실행하면 현재 데이터베이스의 내용이 백업 시점으로 완전히 교체됩니다. 복원 전에 반드시 현재 데이터를 백업하거나 불필요한 데이터임을 확인하십시오.
...
- TAG 테이블은 기간 백업 복원이 지원되지 않습니다. 전체 백업 또는 증분 백업으로 복원하십시오.
...
특정 테이블의 데이터에만 접근하더라도 데이터베이스 전체 마운트를 사용하십시오. 마운트된 데이터베이스에서 원하는 테이블만 선택적으로 조회하면 됩니다.
...
테이블 단위 마운트가 필요한 경우, 데이터베이스 전체를 마운트한 뒤 해당 테이블만 조회하는 방식을 사용하십시오. 마운트는 읽기 전용이므로 다른 테이블에 영향을 주지 않습니다.

<a id="table-types-type-backup-mount"></a>

## 테이블 타입별 백업/마운트 제약

Machbase의 테이블 타입마다 백업과 마운트에 대한 지원 범위가 다릅니다. 백업 계획을 수립하기 전에 해당 테이블 타입의 제약사항을 확인하십시오.
