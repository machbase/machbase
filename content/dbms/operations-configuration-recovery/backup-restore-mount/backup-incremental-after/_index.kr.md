...
증분 백업은 체인 형태로 연결됩니다. 복구 시에는 가장 오래된 전체 백업부터 각 증분 백업을 순서대로 적용해야 합니다.
...
증분 백업으로 복구할 때는 `machadmin -r` 명령을 여러 번 순서대로 실행합니다.
...
> 복구 순서가 잘못되면 데이터 일관성이 보장되지 않습니다. 반드시 전체 백업부터 시간 순서대로 적용하세요.
...
- TAG 테이블은 증분 백업 복원 시 제약이 있을 수 있습니다.
