---
type: docs
title: 'machcollectoradmin으로 Collector 생성/시작/중지/삭제'
weight: 20
---

`machcollectoradmin`은 Collector 인스턴스를 생성·시작·중지·삭제하는 CLI 도구입니다. `-a` 옵션으로 동작(action)을 지정하고 `-n` 옵션으로 Collector 이름을 지정합니다.

## Collector 생성

```bash
machcollectoradmin -a create -n my_collector -f collector_config.xml
```

| 옵션 | 설명 |
|------|------|
| `-a create` | Collector 인스턴스 생성 동작 |
| `-n <이름>` | 생성할 Collector 인스턴스 이름 |
| `-f <파일>` | Collector 설정 파일 경로 (XML 형식) |
