...
| `SESSION_QUERY_TIMEOUT_SEC` | 0 | 아니오 | 쿼리 실행 타임아웃 (0: 무제한) |
...
| 파라미터 | 기본값 | 재시작 필요 | 설명 |
|----------|--------|-------------|------|
| `PROCESS_MAX_SIZE` | 8GB | 예 | 서버 프로세스 최대 메모리 |
...
| 파라미터 | 기본값 | 재시작 필요 | 설명 |
|----------|--------|-------------|------|
| `RS_CACHE_ENABLE` | 1 | 아니오 | Result Cache 활성화 여부 |
| `RS_CACHE_TIME_BOUND_MSEC` | 1000 | 아니오 | 캐시 저장 기준 실행 시간 (밀리초) |
| `RS_CACHE_MAX_MEMORY_SIZE` | 512MB | 아니오 | Result Cache 최대 메모리 |
| `RS_CACHE_MAX_MEMORY_PER_QUERY` | 16MB | 아니오 | 쿼리당 캐시 최대 메모리 |
| `RS_CACHE_MAX_RECORD_PER_QUERY` | 10000 | 아니오 | 쿼리당 캐시 최대 레코드 수 |
| `RS_CACHE_APPROXIMATE_RESULT_ENABLE` | 0 | 아니오 | 근사값 모드 (1: 빠르지만 부정확할 수 있음) |
...
| `TRACE_LOG_LEVEL` | 277 | 예 | 로그 상세 수준 (높을수록 상세) |
...
| `TAG_CACHE_MAX_MEMORY_SIZE` | 512MB | 예 | Tag 캐시 풀당 최대 메모리 |
