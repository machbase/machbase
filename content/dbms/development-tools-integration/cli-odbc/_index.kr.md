...
8.6.0 Standard Edition에서는 connection string의 `DATABASE` 또는 호환 이름 `DBNAME`으로
초기 database를 지정합니다. ODBC catalog는 Machbase database, schema는 owner를
의미합니다.

```text
DSN=MACHBASE;UID=APP_A;PWD=secret;DATABASE=FACTORY_A
```

`SQL_ATTR_CURRENT_CATALOG`로 current database를 조회·변경할 수 있고,
`SQLTables`, `SQLColumns`, `SQLPrimaryKeys` 등 metadata API의 catalog 인자는
조회 대상 database를 선택합니다. metadata API 호출 자체가 session의 current database를
변경하지는 않습니다. 다른 database의 table에는 database `CONNECT`와 table 권한이
모두 필요합니다. 자세한 규칙은 [다중 데이터베이스 운영 가이드](/dbms/operations-configuration-recovery/multi-database/#92-native-c와-odbc)를
참조하십시오.
...
ODBC에서는 `SQLPrimaryKeys()`로 테이블의 PRIMARY KEY 컬럼, 키 이름, 키 순서를 조회합니다.
TRANSACTION·LOOKUP·VOLATILE 테이블의 선언된 PK와 TAG 테이블의 논리적 PK인 `NAME`을
반환하며, 일반 LOG 테이블에는 PK 행을 반환하지 않습니다.
...
SQLPrimaryKeys(stmt,
              NULL, 0,             /* catalog */
              (SQLCHAR *)"SYS", SQL_NTS,
              (SQLCHAR *)"ACCOUNT", SQL_NTS);

/* SQLFetch 후 TABLE_NAME, COLUMN_NAME, PK_NAME, KEY_SEQ 등을 확인합니다. */
SQLFreeStmt(stmt, SQL_CLOSE);
```

테이블 소유자가 `SYS`가 아니어도 해당 소유자의 PK를 조회합니다. SQLCLI에는 PRIMARY KEY를
반환하는 별도 표준 API가 없으므로 기존 `DescribeCol` 계약은 변경되지 않았습니다. `machsql`
사용자는 `DESC table_name`의 `[ PRIMARY KEY ]` 섹션에서 PK 이름, 컬럼, key sequence를
확인합니다.

Machbase 8.6.0의 CMI 4.0.3 메타데이터를 협상한 경우 SELECT 결과 컬럼에도 PK 플래그가
