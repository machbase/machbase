...
VOLATILE 테이블은 다른 테이블 타입과 동일하게 `SELECT` 문으로 조회한다. 데이터와 인덱스가 메모리에 있으므로 최신 상태 캐시, 임시 집계, 작업 상태 조회처럼 반복 조회가 많은 용도에 적합하다.
...
PRIMARY KEY가 있는 VOLATILE 테이블은 키 조건으로 단건 데이터를 조회한다.
...
최신 상태 캐시는 대부분 PRIMARY KEY 조회로 구성한다. `ON DUPLICATE KEY UPDATE`로 같은 키의 값을 계속 갱신하면 조회 쿼리는 항상 최신 상태만 읽는다.

<a id="query-volatile-index"></a>

## 인덱스 기반 조회

PRIMARY KEY 외 컬럼을 조건이나 범위로 자주 조회하면 보조 인덱스를 생성한다.
...
모든 인덱스는 메모리를 사용한다. 조회에 사용하지 않는 인덱스를 과도하게 만들면 메모리 사용량과 갱신 비용이 증가한다.

<a id="query-volatile-temporary-analysis"></a>

## 임시 집계 조회

짧은 주기의 집계 결과를 VOLATILE 테이블에 저장하면 대시보드나 알람 판단에서 반복 계산을 줄일 수 있다.
...
집계 결과를 장기 보관해야 하면 VOLATILE 테이블에만 두지 말고 LOG 또는 RDB 테이블로 주기적으로 복사한다.

<a id="query-volatile-limitations"></a>

## 조회 시 주의사항

- 서버 재시작 후 VOLATILE 데이터는 비어 있을 수 있으므로 조회 결과가 0건인 상황을 처리한다.
- PRIMARY KEY 조회가 중심이면 PRIMARY KEY를 반드시 지정한다.
- 범위 조회나 정렬 조건에 자주 쓰는 컬럼은 보조 인덱스를 검토한다.
- JSON 컬럼은 VOLATILE 테이블에서 사용할 수 없다.
- 중요한 원본 데이터는 TAG, LOG, LOOKUP, RDB 같은 영속 테이블에 저장하고 VOLATILE은 캐시로 사용한다.
