...
VOLATILE 테이블의 상태 캐시와 임시 집계 패턴을 다룬다. 이 절의 패턴은 원본 데이터를 영속 테이블에 보관하고, VOLATILE 테이블에는 빠른 조회를 위한 최신 상태나 중간 결과만 유지하는 방식이다.

<a id="pattern-volatile-latest-state"></a>

## 최신 상태 캐시

장비나 센서의 최신 상태만 빠르게 조회해야 하면 PRIMARY KEY를 장비 ID로 둔 VOLATILE 테이블을 사용한다.
...
원본 이력은 TAG 또는 LOG 테이블에 별도로 저장한다.

<a id="pattern-volatile-temporary-aggregation"></a>

## 임시 집계 테이블

짧은 주기의 집계 결과를 임시로 유지할 때 VOLATILE 테이블을 사용한다.
...
집계 결과를 장기 보관해야 하면 주기적으로 RDB 또는 LOG 테이블에 복사한다.
...
배치나 수집 작업의 현재 상태를 공유해야 할 때도 VOLATILE 테이블을 사용할 수 있다.
...
운영 화면은 이 테이블을 조회해 현재 상태를 표시한다. 작업 이력이나 감사 로그가 필요하면 별도 LOG 또는 RDB 테이블에 기록한다.

<a id="pattern-volatile-rebuild"></a>

## 재구성 패턴

VOLATILE 테이블은 재시작 후 비어 있으므로, 원본 데이터에서 다시 구성할 수 있어야 한다.
...
실제 재구성 쿼리는 원본 테이블의 중복 처리 기준과 최신값 판정 기준에 맞게 작성한다.

<a id="pattern-volatile-guidelines"></a>

## 설계 지침

- 원본 데이터는 영속 테이블에 보관한다.
- VOLATILE 테이블에는 최신 상태나 재계산 가능한 결과만 저장한다.
- PRIMARY KEY는 재구성 시에도 같은 값을 만들 수 있어야 한다.
- 보존이 필요한 임시 집계는 주기적으로 영속 테이블에 플러시한다.
- 서버 재시작 후 캐시가 비어 있어도 애플리케이션이 동작하도록 처리한다.
