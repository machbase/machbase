...
| --node-type | 노드 타입을 지정합니다.<br>coordinator / deployer / lookup / broker / warehouse 다섯 가지가 있습니다. | coordinator       |
...
| 옵션          | 설명                                                                                    | 예시              |
| ------------- | --------------------------------------------------------------------------------------- | ----------------- |
| --remove-node | 삭제할 노드 이름을 "IP:PORT"로 지정합니다.<br>PORT 값은 CLUSTER_LINK_PORT_NO 값입니다. | 192.168.0.83:5111 |
...
| --node-type | 노드 타입을 지정합니다.<br>coordinator / deployer / lookup / broker / warehouse 다섯 가지가 있습니다. | deployer          |
...
| --file-name   | 패키지 파일의 전체 경로와 파일 이름을 지정합니다.<br>이 패키지는 broker와 warehouse 설치용이므로 MWA 파일을 제외한 경량 패키지를 지정합니다. | /home/machbase/machbase-ent-x.y.z.official-LINUX-X86-64-release-lightweight.tgz |
