...
TAG 테이블에 직접 삽입된 시계열 값(value)은 현재 빌드에서 UPDATE할 수 없습니다.
...
잘못 입력된 값을 정정해야 하면 새 데이터로 보정값을 입력하거나, 별도 LOG/RDB 테이블에
정정 이력을 남기고 조회 시 결합합니다.
...
| 시계열 값 (value, 사용자 추가 컬럼) | `UPDATE tag SET value = ...` | X (현재 미지원) |
| 태그 이름(PK) | 변경 불가 | X |

## 주의 사항

- 메타데이터 UPDATE는 해당 name의 **모든 시계열 데이터**에 영향을 줍니다. (name 단위 속성이므로)
- 메타데이터 변경은 태그 속성 변경이며, 이미 적재된 시계열 값 자체를 수정하지 않습니다.
