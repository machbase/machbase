...
| RENAME COLUMN | O | X | O | O | O |
| MODIFY COLUMN | O | O | X | O | O |
| RENAME TABLE | O | X | O | X | X |
...
```sql
ALTER TABLE web_access RENAME COLUMN resp_time TO response_ms;
ALTER TABLE orders RENAME COLUMN note TO memo;
```

## MODIFY COLUMN

현재 VARCHAR 컬럼의 크기 확장, MINMAX_CACHE_SIZE 설정, NOT NULL 제약 변경을 지원합니다.
...
-- NOT NULL 제약 추가
ALTER TABLE orders MODIFY COLUMN status NOT NULL;
