...
LOG 테이블의 `TEXT` 타입 컬럼은 전문 검색(full-text search)을 지원합니다. `SEARCH` 함수를 사용하여 특정 단어나 패턴을 포함하는 레코드를 빠르게 찾을 수 있습니다.
...
FROM app_log
WHERE SEARCH(message, 'error');

-- 여러 단어 AND 조건
SELECT _arrival_time, message
FROM app_log
WHERE SEARCH(message, 'connection refused');
...
| `SEARCH(col, 'keyword')` | 역인덱스 사용 | 단어 단위 검색, 고성능 |

## 주의사항

- `TEXT` 컬럼은 최대 64KB 저장 가능합니다.
