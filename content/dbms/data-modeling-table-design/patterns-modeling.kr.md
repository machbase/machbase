...
서버를 재시작하면 VOLATILE 테이블의 데이터는 사라지지만 테이블 정의는 남습니다. 원본
TAG 테이블에서 같은 최근 2시간 통계를 재계산합니다. 아래 CREATE는 최초 구축처럼 테이블이
없을 때만 테이블을 만듭니다. 원본 보존 기간 안에 재계산할 데이터가 남아 있어야 합니다.
