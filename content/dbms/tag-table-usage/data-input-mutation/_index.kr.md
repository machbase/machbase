...
TAG 테이블에 데이터를 입력하는 방법은 INSERT 문, CSV 임포트, RESTful API, SDK 네 가지가 있다. 데이터 규모와 수집 환경에 따라 적합한 방법이 다르므로 각각의 특징과 사용법을 살펴본다.
...
소규모 데이터셋이나 대화형 테스트에 적합한 가장 간단한 입력 방법이다.
...
`csvimport` 도구로 CSV 파일의 대량 데이터를 빠르게 로드한다.

#### CSV 파일 형식

태그 이름, 타임스탬프, 값이 포함된 CSV 파일(`data.csv`)을 준비한다.
...
HTTP 요청으로 데이터를 삽입한다. IoT 디바이스나 웹 애플리케이션에서 주로 활용한다.
...
1. **태그 메타데이터 등록**: 없는 태그 이름은 자동 등록되지만, 메타데이터 값을 명시해야 할 때는 먼저 등록한다. `_LAST_UPDATE_TIME` 은 서버가 자동 관리하므로 직접 입력하지 않는다
2. **배치 작업 사용**: 대규모 데이터셋은 CSV 임포트 또는 배치 API 호출을 사용한다
3. **에러 처리**: 반환 값을 확인하고 에러를 로그에 기록한다
4. **시간 정밀도**: 데이터 전체에서 타임스탬프 정밀도를 일관되게 유지한다
5. **데이터 검증**: 태그 이름과 타임스탬프가 운영 규칙을 따르는지 확인한다

### 성능 팁

- **CSV 임포트**: 대량 데이터(수백만 행)에 가장 빠르다
- **배치 삽입**: 여러 INSERT 문을 트랜잭션으로 그룹화한다
- **병렬 로딩**: 병렬 수집을 위해 여러 csvimport 프로세스를 사용한다
- **준비된 문**: SDK에서 파라미터화된 쿼리로 성능을 높인다
