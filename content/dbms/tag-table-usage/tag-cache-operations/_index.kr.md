...
TAG 테이블의 메타데이터 캐시를 관리하고 크기를 조정하는 방법을 다룹니다.
...
TAG 테이블의 태그 이름, 메타데이터 컬럼 값 등은 메모리에 캐시되어 조회 성능을 높입니다. 태그 수가 많을수록 캐시 효과가 크며, `TAG_CACHE_MAX_MEMORY_SIZE` 파라미터로 크기를 제어합니다.

`FLUSH TAG_CACHE`를 실행하면 현재 캐시된 TAG 메타데이터가 모두 비워지고, 이후 조회 시 캐시가 다시 채워집니다.
