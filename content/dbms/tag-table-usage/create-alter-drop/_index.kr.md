...
한 테이블에는 축 컬럼을 하나만 정의합니다. 태그명은 첫 번째, 축은 두 번째 컬럼이며
`SUMMARIZED`를 지정하면 세 번째 컬럼에 사용합니다. 다음 두 테이블은 이 페이지의
독립 실습용입니다. 기존 객체가 없는지 확인한 뒤 순서대로 실행합니다. 태그명 컬럼은 `PRIMARY KEY`이며, 시간축과
...
`SUMMARIZED`는 지원 숫자 타입이나 JSON에 지정할 수 있습니다. 숫자 요약과 JSON
요약은 조회·집계 규칙이 다르며, ARRAY에는 지정할 수 없습니다. 이 속성만으로 ROLLUP이
생성되지는 않습니다. ROLLUP 생성은 [6장](../../tag-rollup-usage/)을 참고합니다. 위치·단위처럼
