...
일반 사용자
  ├─ 데이터베이스 권한 (DB 전체 범위)
  │    CREATE, DROP, ALTER, BACKUP, MOUNT, DDL, ALL
  └─ 테이블 권한 (특정 테이블 범위)
...
```sql
-- 데이터베이스 권한 전체 (CREATE, DROP, ALTER, BACKUP, MOUNT)
...
대상이 `MACHBASEDB`이면 데이터베이스 관리 권한 전체, 특정 테이블이면 해당 테이블의 DML 권한 전체를 의미합니다.
