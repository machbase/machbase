...
Machbase의 보안 모델은 **사용자 계정**, **권한(GRANT/REVOKE)**, **접속 제어(IP/인증)** 세 요소가 계층적으로 결합된 구조입니다. 이 구조를 이해하면 운영 환경에서 필요한 보안 정책을 체계적으로 수립할 수 있습니다.
...
Machbase를 설치하면 `SYS` 계정이 자동으로 생성됩니다. SYS는 슈퍼유저로서 모든 데이터베이스 작업을 수행할 수 있으며, 다른 사용자를 생성하고 권한을 부여하는 관리자 역할을 담당합니다.
...
AUTH KEY를 사용하면 비밀번호 유출 위험 없이 서비스 계정을 안전하게 운영할 수 있습니다. 자세한 설정 방법은 [AUTH KEY 인증](../authentication-auth-key/)을 참고하세요.

## 최소 권한 원칙

운영 환경에서는 다음 원칙을 적용하기를 권장합니다.
