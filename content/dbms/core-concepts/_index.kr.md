...
비영구 메모리에서 UPDATE/DELETE가 필요한가요?
    예 → Volatile 테이블

하나의 primary key로 접근하는 영구 참조 데이터인가요?
    예 → Lookup 테이블

인덱스 predicate가 필요한 범용 디스크 row인가요?
    예 → RDB 테이블
```
...
| Lookup    | 초당 수백 건 | 빠름 | 키 기반 | 영구 데이터 + 메모리 인덱스 |
| RDB       | 워크로드에 따라 다름 | 빠름 | 가능 | 디스크 |
