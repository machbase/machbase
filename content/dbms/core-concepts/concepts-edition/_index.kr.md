...
하나의 프로세스로 동작하는 단일 노드 구성입니다. 설치와 운영이 간단하고, 소규모에서 중규모 시계열 데이터를 처리하는 데 충분한 성능을 제공합니다.
...
초당 수십만 건의 입력과 수 TB 규모의 데이터를 단일 서버에서 처리합니다.

### Cluster Edition

여러 서버에 데이터를 분산 저장하고 병렬로 처리합니다. 내부적으로 `EDITION_CLUSTER` 컴파일 플래그로 분리된 별도 빌드입니다.
