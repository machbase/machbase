...
- ROLLUP, STREAM, Retention Policy, Backup/Mount 등 대부분의 기능 지원

초당 수십만 건의 입력과 수 TB 규모의 데이터를 단일 서버에서 처리할 수 있습니다.

## Cluster Edition

Cluster Edition은 여러 서버에 데이터를 분산 저장하고 병렬로 처리합니다. 내부적으로 `EDITION_CLUSTER` 컴파일 플래그로 분리된 별도 빌드이며, 네 종류의 노드로 구성됩니다.
