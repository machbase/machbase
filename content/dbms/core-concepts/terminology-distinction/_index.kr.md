...
| 대상 테이블 | LOG, TAG | LOG, TAG, LOOKUP, VOLATILE, TRANSACTION | LOG, TRANSACTION |
...
key equality 조건을, TRANSACTION 테이블은 일반 `WHERE` 조건을 사용합니다. LOOKUP의 조건 없는 DELETE는
모든 행을 삭제합니다.
...
지원 대상 테이블의 모든 데이터를 즉시 삭제합니다. WHERE 조건 없이 한 줄로 전체 데이터가 제거됩니다. LOG 테이블과 TRANSACTION 테이블에 사용합니다.
...
| LOG/TRANSACTION 테이블 전체 초기화 (개발/테스트) | TRUNCATE |
| LOG/TRANSACTION 테이블 전체 즉시 폐기 | TRUNCATE |
...
형식을 지원하며, 스키마 파일로 컬럼 매핑, 구분자와 날짜 형식을 제어합니다. TRANSACTION 테이블은
