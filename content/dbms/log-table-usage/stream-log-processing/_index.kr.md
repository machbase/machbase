...
STREAM 기능을 사용하여 LOG 테이블의 데이터를 TAG 테이블에 자동 적재하는 방법을 다룬다.
...
STREAM을 이용해 LOG 테이블에 삽입되는 데이터를 실시간으로 TAG 테이블에 자동 적재하는 파이프라인이다.

외부 수집기(Collector, OPC-UA 클라이언트, MQTT 브로커 등)가 LOG 테이블에만 데이터를 기록할 수 있는 환경에서, TAG 테이블의 ROLLUP 집계와 시계열 분석 기능을 함께 활용하기 위한 패턴이다. 애플리케이션 코드 변경 없이 LOG에서 TAG로의 변환을 데이터베이스 내부에서 처리한다.
...
ROLLUP 집계 및 시계열 분석을 위한 TAG 테이블을 생성한다.
...
TAG 테이블은 `name` 컬럼으로 각 센서 채널을 구분하므로, STREAM에서 채널별로 태그 이름을 지정한다.

### 2단계: STREAM 생성

`EXEC STREAM_CREATE` 프로시저로 STREAM을 생성한다. LOG 테이블의 신규 레코드를 감지하여 지정된 INSERT 쿼리를 자동 실행한다.
...
> **문자열 이스케이프**: STREAM 쿼리 내 문자열 리터럴은 작은따옴표를 두 번 겹쳐(`''`) 이스케이프한다.

### 3단계: STREAM 시작

생성된 STREAM을 시작한다. 이후부터 LOG 테이블에 새 레코드가 삽입될 때마다 STREAM이 자동으로 동작한다.
...
`v$streams` 가상 테이블로 실행 중인 STREAM의 상태를 확인한다.
...
LOG 테이블에 데이터를 삽입하면 STREAM이 즉시 TAG 테이블로 전달한다.
...
실제 산업 환경에서는 하나의 LOG 테이블에 수십 개의 센서 채널이 포함되는 경우가 많다. 각 채널마다 STREAM을 생성하여 개별 TAG로 분산한다.
...
`end_rid`가 LOG 테이블의 총 레코드 수와 동일해지면 모든 데이터가 처리된 상태다.

### 6단계: 성능 고려사항

#### STREAM 처리 지연 모니터링

STREAM은 비동기로 동작하므로 삽입 시점과 TAG 테이블 반영 시점 사이에 약간의 지연이 발생할 수 있다.
...
STREAM 개수가 많을수록 LOG 테이블 삽입 시 처리 부하가 증가한다. 채널 수가 100개 이상인 경우 다음 대안을 고려한다.
...
중지된 STREAM은 `v$streams`에서 `STATE = STOPPED`로 표시된다. `STREAM_START`로 재시작하면 중지 시점 이후의 레코드부터 처리를 재개한다.

#### STREAM 삭제

불필요한 STREAM은 먼저 중지한 뒤 삭제한다.
