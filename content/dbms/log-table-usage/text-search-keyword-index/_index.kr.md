...
| 단어 단위 | O (문자 종류에 따른 토큰 분리) | X (패턴 매칭) |
| 처리 방식 | 인덱스로 후보 검색 | 대상 문자열에 패턴 적용 |
...
`ESEARCH`는 KEYWORD 인덱스를 이용하는 `%` 와일드카드 확장 검색입니다. 실제 검색 비용은
패턴에 일치하는 키워드와 행 수에 따라 달라지므로 대표 문자열로 결과와 실행 시간을 확인합니다.
...
|------|---------|------|
| KEYWORD 인덱스 | 필요 | 불필요 |
| 처리 방식 | 키워드 인덱스로 후보 검색 | 대상 문자열에 패턴 적용 |
...
- LOG 테이블의 VARCHAR/TEXT 컬럼에서 단어 검색이 반복되면 KEYWORD 인덱스와 `SEARCH` 또는
  `ESEARCH`를 검토합니다. 단어 검색과 부분 문자열 검색은 결과 의미가 다를 수 있습니다.
...
- 영문은 공백·구두점 등을 기준으로 단어를 분리하고, 한국어·일본어 등은 위의 다국어
  검색 규칙을 적용합니다.
