...
LOG 테이블은 append 중심 테이블입니다. 데이터는 시간 순서로 계속 입력되고, 보존 기간이 지난 데이터는 오래된 영역부터 정리합니다.

임의 위치의 데이터를 삭제하는 것은 불가능하며, 임의 위치에서 마지막(가장 오래된 로그) 레코드까지 연속적으로 삭제하는 방식만 지원합니다. 로그 데이터의 특성을 활용한 정책으로, 공간 확보를 위해 파일을 순서대로 삭제하는 행위를 DB 형식으로 표현한 것입니다.

사용할 수 있는 표현식은 다음과 같습니다.
...
운영 환경에서는 삭제 전 같은 기준으로 보존 범위를 확인합니다.
...
LOG 테이블은 보존 기간을 기준으로 운영하는 것이 일반적입니다.

| 정책 | 예시 |
|------|------|
| 기간 기준 보존 | 최근 90일만 유지 |
| 건수 기준 보존 | 최근 1억 건만 유지 |
| 원본 보존 후 집계 | 원본은 30일, 집계 결과는 TRANSACTION 또는 TAG에 장기 보존 |
| 백업 후 삭제 | 백업 완료 후 오래된 구간 삭제 |

기간 기준 삭제는 `BEFORE` 또는 `EXCEPT ... DAY` 형식으로 표현합니다.
...
장기 보관이 필요한 로그는 삭제 전에 백업합니다.

```sql
BACKUP DATABASE INTO DISK = '/backup/machbase_log_20260101';
```

백업을 마운트해 필요한 구간을 조회할 수 있는지 확인한 뒤 삭제 작업을 수행합니다.
...
- 입력량 증가에 따라 테이블 크기와 보존 기간을 주기적으로 조정합니다.
- 삭제 전 대상 건수와 시간 범위를 확인합니다.
- 백업이 필요한 데이터는 백업 완료 후 삭제합니다.
- 잘못 입력된 특정 행을 수정해야 하는 모델이면 LOG가 아니라 TRANSACTION 또는 LOOKUP 사용을 검토합니다.
- `_arrival_time` 기준 보존과 실제 이벤트 시각 기준 보존을 혼동하지 않습니다.
