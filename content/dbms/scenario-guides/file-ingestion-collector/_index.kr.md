...
Machbase Collector는 로컬 파일, TCP/UDP 소켓, 시리얼 포트 등 다양한 외부 데이터 소스에서 데이터를 수집하여 Machbase에 자동으로 적재하는 컴포넌트입니다. 데몬 형태로 상시 동작하며, 설정 파일을 통해 수집 소스와 적재 대상 테이블을 연결합니다. 이 시나리오에서는 CSV 파일 감시 수집과 TCP 소켓 수집을 중심으로 실전 설정 방법을 설명합니다.
