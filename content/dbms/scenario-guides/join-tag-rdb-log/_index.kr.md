---
type: docs
title: 'TAG + RDB + LOG 조인 대시보드'
weight: 40
---

Machbase는 TAG(시계열), RDB(관계형), LOG(이벤트) 세 가지 테이블 유형을 하나의 SQL로 조인할 수 있습니다. 이 시나리오는 장비별 최신 센서값, 장비 마스터 정보, 시스템 이벤트 로그를 통합해 운영 대시보드를 구성하는 방법을 설명합니다.
...
센서값 이상과 시스템 이벤트 발생의 시간적 연관성을 분석합니다.
...
현재 기준으로 임계값을 초과한 장비와 담당자를 즉시 확인합니다.
...
> TAG 테이블과 LOG 테이블을 시간 범위로 조인할 때는 검색 범위(`BETWEEN`)를 최대한 좁혀 전체 스캔을 방지하세요.
