...
백업 이미지를 마운트된 데이터베이스로 연결하면 현재 데이터베이스를 덮어쓰지 않고 과거 데이터를
읽을 수 있습니다. 이 작업은 파일 시스템과 데이터베이스 전역 상태에 영향을 주므로 격리 환경에서
...
여유 공간이 충분한지 먼저 확인합니다. 운영 데이터베이스의 백업 정책을 시험 경로로 덮어쓰지
마십시오.
...
마운트된 데이터베이스는 읽기 전용 분석에 사용합니다. 현재 데이터베이스의 값과 비교할 때는 어느 쪽
데이터베이스의 객체인지 알 수 있도록 세 부분 이름을 명시합니다.
...
UMOUNT 전에 마운트된 데이터베이스를 사용하는 세션과 쿼리가 없는지 확인합니다. MOUNT 조회 성공은
백업 검증의 한 단계일 뿐입니다. 복구 가능성을 보장하려면 별도 환경의 복원 시험과 업무
검증 쿼리까지 수행하십시오.
