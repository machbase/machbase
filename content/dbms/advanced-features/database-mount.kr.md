...
> **참고**: Machbase 8.5 이상에서는 일반 사용자가 `MOUNT DATABASE` 또는 `UNMOUNT DATABASE`를 실행하려면 `GRANT MOUNT ON machbasedb TO user_name;` 권한이 필요합니다. 자세한 권한 부여 방법은 [사용자 관리](../sql-reference/user-manage/#grantrevoke)의 `GRANT/REVOKE`를 참고하세요.
