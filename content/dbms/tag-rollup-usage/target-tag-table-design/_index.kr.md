...
1. **가장 작은 단위부터**: 조회에 필요한 최소 구간을 표현할 수 있도록 최하위 주기를 정합니다.
2. **배수 관계 유지**: 상위 ROLLUP 주기는 하위 주기의 정수배여야 합니다.
3. **계층 수 최소화**: 실제로 필요한 조회 구간만 지원하도록 계층을 구성합니다.

```
초 단위 분석 필요: 1SEC → 1MIN → 1HOUR
분 단위 분석 충분: 1MIN → 1HOUR
대용량 장기 보관:  5MIN → 1HOUR → 조회 시 day 단위 재집계
```
...
모든 태그가 각 구간에 데이터를 갖는다고 가정하면 논리적인 태그·시간 버킷 수는 다음과 같습니다.

태그·시간 버킷 수 ≈ (전체 기간 / ROLLUP 주기) × 태그 개수

예를 들어 태그 1만 개의 1초 구간을 365일 보관하면 약 3,154억 버킷입니다. 실제 저장량은
빈 구간, 부분 집계 행, 집계 컬럼 수와 압축률의 영향을 받으므로 표본 적재 결과로 산정합니다.

> 태그가 많을수록 ROLLUP 테이블 크기가 비례해 커집니다. 필요한 컬럼과 주기만 선택적으로 만드십시오.

### 주의사항

- 새 ROLLUP은 원본에 남아 있는 기존 데이터도 처리하므로 초기 집계에 시간이 걸릴 수 있습니다.
  즉시 결과를 확인하려면 `ROLLUP_FORCE`로 현재 입력분 처리를 기다리고 `SHOW ROLLUPGAP`을
  확인합니다. 이미 집계한 원본을 수정한 경우에는 별도로 `ROLLUP_REBUILD`를 검토합니다.
