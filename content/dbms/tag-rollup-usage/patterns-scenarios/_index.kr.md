...
산업 IoT 환경에서는 수십~수천 개의 센서가 초당 수만 건의 데이터를 발생시킵니다. 이 시나리오에서는 Machbase TAG 테이블에 센서 데이터를 저장하고 ROLLUP 집계로 분/시간/일 단위 통계를 조회하는 흐름을 단계별로 다룹니다.
...
ROLLUP 테이블은 원시 데이터를 시간 단위로 자동 집계합니다. 미리 집계해 두므로 대시보드 조회 시 수백만 건의 원시 데이터를 스캔하지 않아도 됩니다.
