...
`ENABLED`가 꺼져 있거나 `RUN_STATE`가 비정상 상태로 유지되는 경우입니다.
...
ROLLUP은 설정된 주기마다 실행되므로 방금 입력된 데이터가 즉시 반영되지 않습니다. 이는 정상 동작입니다.

즉시 반영이 필요하면 수동으로 실행합니다.
...
이미 저장된 ROLLUP 값이 잘못된 경우 특정 시간 범위만 재계산할 수 있습니다. **Standard Edition 전용** 기능입니다.
...
주기가 너무 길어 최신 데이터 반영이 늦다면 ROLLUP 설정을 재검토하거나 `FLUSH ROLLUP`을 수동으로 실행합니다.
