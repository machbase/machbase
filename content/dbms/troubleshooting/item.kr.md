...
클라이언트가 보고한 전체 오류, 대상 데이터베이스·테이블, 입력 방식, 마지막 성공 행을 먼저
기록합니다. 오류 코드 의미는 [오류 코드 사전](/dbms/reference/error-codes/)에서
