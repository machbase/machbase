...
weight: 60
---
Machbase는 TAG 데이터를 자동으로 집계하는 ROLLUP과 쿼리 결과를 다른 테이블에 자동으로 적재하는 STREAM 기능을 제공합니다. 이 섹션은 두 기능이 정상적으로 동작하지 않을 때 원인을 진단하고 복구하는 방법을 다룹니다.
...
STREAM은 특정 소스 테이블의 데이터를 쿼리하여 대상 테이블에 자동으로 적재하는 기능입니다. STREAM이 중지되거나 데이터가 적재되지 않을 때 원인을 진단하고 복구하는 방법을 설명합니다.
...
STREAM이 정상적으로 데이터를 적재하는지 확인합니다.
