...
Append API로 데이터 입력이 실패하면 원인은 대부분 연결 상태, 테이블 정의, 데이터 형식, 디스크 상태 중 하나입니다. 오류 메시지를 단서로 원인을 좁혀 나가십시오.
...
machloader CSV 가져오기가 실패하면 대부분 파일 경로, 컬럼 수, 인코딩, 구분자, 날짜 형식 중 하나가 원인입니다. 오류 로그 파일을 먼저 확인하고 해당 옵션을 조정하십시오.
...
Collector 데이터 수집이 멈추거나 지연될 때는 트레이스 로그가 첫 번째 단서입니다. 소스 연결 실패, 큐 오버플로우, Machbase 연결 실패, 데이터 형식 오류 중 어느 쪽인지 로그로 판단합니다.
