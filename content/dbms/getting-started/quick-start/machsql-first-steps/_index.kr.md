...
`machsql`은 Machbase DBMS에 SQL을 전달하는 기본 명령행 클라이언트입니다. 사용자가 SQL을 작성하면 `machsql`이 이를 서버로 전달하고, 서버는 실행 결과를 돌려줍니다. SQL 파일을 `-f` 옵션으로 넘기면 파일 안의 SQL을 일괄 실행하고 결과를 출력합니다.

대화형으로 접속해 여러 문장을 직접 입력하는 방식과, SQL 파일을 지정해 일괄 실행하는 방식 모두 지원합니다. 이 장의 예제는 반복 검증이 편한 파일 실행 방식을 사용합니다.
