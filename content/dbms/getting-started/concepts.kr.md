...
Machbase는 여러 테이블 타입을 제공합니다. **올바른 것을 선택하는 것이 중요합니다!**
...
특정 레코드를 UPDATE 또는 DELETE해야 하나요?
    예 → 메모리/비영구/key 기반이면 VOLATILE TABLE 사용

하나의 primary key로 접근하는 영구 참조/마스터 데이터인가요?
    예 → LOOKUP TABLE 사용

일반 인덱스 또는 선택적 primary key가 필요한 디스크 row 테이블인가요?
    예 → RDB TABLE 사용
```
...
- 최신 데이터가 먼저 반환됨
- KEYWORD 인덱스 생성 후 `SEARCH` 키워드로 전문 검색
- 유연한 스키마
...
### 3. 휘발성 테이블 - 비영구 메모리 데이터용

**사용 시기**: 메모리에서 빠른 INSERT/UPDATE/DELETE가 필요하고 재시작 시 데이터 손실을 허용할 때
...
**구조**: 하나의 필수 `PRIMARY KEY`를 포함한 유연한 스키마
...
**사용 시기**: 하나의 primary key로 접근하는 영구 참조/마스터 데이터 저장
...
- primary-key predicate 기반 전체 DML
- 시계열 테이블과 조인
...
### 5. RDB 테이블 - 범용 디스크 row용

**사용 시기**: `INSERT`, `UPDATE`, `DELETE`, 인덱스 기반 `SELECT`가 필요한 디스크 기반 row 데이터 저장

**최적 용도**:
- 단일 key Lookup 모델에 맞지 않는 일반 row 스키마
- 장비 마스터 데이터
- 알람 룰 및 현재 알람 상태
- 작업 큐
- Tag 또는 Log 데이터와 조인하는 차원 테이블

**구조**: 선택적 `PRIMARY KEY`를 포함한 유연한 디스크 row 스키마

**주요 기능**:
- 전체 DML 지원
- 영구 저장
- 일반, unique, primary key, JSON path 인덱스
- 시계열 테이블과 조인
...
| 기능 | 태그 테이블 | 로그 테이블 | 휘발성 테이블 | 룩업 테이블 | RDB 테이블 |
|---------|-----------|-----------|----------------|--------------|-----------|
| **용도** | 센서 데이터 | 로그/이벤트 데이터 | 비영구 메모리 상태 | primary-key 참조 데이터 | 범용 디스크 row |
| **삽입 속도** | 백만/초 | 백만/초 | 만/초 | 백/초 | 워크로드에 따라 다름 |
| **UPDATE 지원** | 아니오* | 아니오 | 예 | 예 | 예 |
| **DELETE 지원** | 시간 기반 | 시간 기반 | 키 기반 | 키 기반 | 예 |
| **저장소** | 디스크 | 디스크 | 메모리 | 영구 데이터 + 메모리 인덱스 | 디스크 |
| **스키마** | 고정 패턴 | 유연 | 유연 | 유연 | 유연 |
| **최적 쿼리** | ID + 시간 | 시간 기반 | 키 기반 | primary key 기반 | 인덱스 predicate |
| **데이터 영속성** | 예 | 예 | **아니오** | 예 | 예 |
...
- 메모리/비영구/key 기반 데이터는 Volatile 테이블 사용
- 영구 참조 데이터와 단일 primary-key 접근은 Lookup 테이블 사용
- 디스크 row 데이터와 여러 인덱스 또는 일반 predicate가 필요하면 RDB 테이블 사용
...
- **룩업 테이블**: 메모리 primary-key 인덱스
- **RDB 테이블**: 일반, unique, primary key, JSON path 조회용 디스크 테이블 인덱스

대부분의 사용자는 인덱스를 수동으로 관리할 필요가 없습니다!

## Rollup 테이블 (태그 테이블만)

태그 테이블은 `WITH ROLLUP`으로 생성하거나 `CREATE ROLLUP`으로 rollup을 정의하면 통계를
생성합니다:
...
- **비영구 실시간 key 기반 업데이트** → 휘발성 테이블
- **하나의 primary key로 접근하는 영구 참조 데이터** → 룩업 테이블
- **범용 디스크 row 데이터와 workflow 데이터** → RDB 테이블
