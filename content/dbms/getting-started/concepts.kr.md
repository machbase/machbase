...
영구 row UPDATE/DELETE와 인덱스 조회가 필요한가요?
    예 → RDB TABLE 사용
```
...
### 5. RDB 테이블 - 영구 row 상태용

**사용 시기**: `INSERT`, `UPDATE`, `DELETE`, 인덱스 기반 `SELECT`가 필요한 영구 운영 상태 저장
...
**구조**: 선택적 `PRIMARY KEY`를 포함한 유연한 스키마
...
- **RDB 테이블**: 일반, unique, primary key, JSON path 조회용 RDB 인덱스
...
- **실시간 업데이트 필요** → 휘발성 테이블
- **구성, 참조 데이터** → 룩업 테이블
- **영구 row 상태와 workflow 데이터** → RDB 테이블
