...
빠른 시작을 마쳤다면 이제 본격적인 갈림길에 선 것입니다. 같은 Machbase DBMS를 사용하더라도 산업 IoT 센서 값을 저장하려는 사용자, 금융 틱 데이터를 적재하려는 사용자, 로그를 분석하려는 사용자, 애플리케이션을 연결하려는 사용자가 읽어야 할 다음 문서는 서로 다릅니다. Machbase DBMS 매뉴얼은 기능 이름보다 사용자의 설계 질문을 기준으로 읽는 것이 효과적입니다.
...
TAG 테이블은 센서나 설비의 계측값을 태그 이름과 시간축 기준으로 정리하는 데 사용합니다. `sensor01`이라는 태그 이름 아래에 특정 시각의 값을 기록하고, 필요할 때 태그 이름과 시간 범위로 다시 조회하는 구조입니다.

TAG 테이블에 입력한 데이터를 즉시 조회하려면 `TABLE_FLUSH`를 먼저 실행해야 합니다. 이 동작을 초반에 확인해 두면 이후 TAG 관련 문서를 읽을 때 혼동을 줄일 수 있습니다.
...
`sensor01`이 출력되면 TAG 테이블의 기본 입력과 조회가 정상적으로 동작한 것입니다. LOG 예제가 "어떤 일이 일어났는가"를 기록했다면, TAG 예제는 "어느 대상의 값이 언제 얼마였는가"를 기록합니다. 이 차이를 염두에 두면 다음 장의 테이블 설계 문서를 훨씬 수월하게 읽을 수 있습니다. 재실행 중 `DBMS_GS_TAG`가 이미 존재한다는 오류가 나면 `DROP TABLE DBMS_GS_TAG;`를 실행한 뒤 다시 시작합니다.
