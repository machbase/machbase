...
Machbase DBMS는 센서 계측값, 설비 이벤트, 애플리케이션 로그, 금융 틱처럼 시간에 따라
...
체결, 서비스 시작처럼 어떤 일이 발생한 사실을 기록합니다. 장비 이름, 설치 위치와
...
데이터의 산업 분야만으로 테이블을 결정하지는 않습니다. 같은 금융 틱이라도 종목별
값의 변화가 중심인지, 여러 필드로 구성된 수신 이벤트의 검색이 중심인지에 따라 설계가
달라집니다. 변경·조인·보관 요구사항을 포함한 최종 선택은
