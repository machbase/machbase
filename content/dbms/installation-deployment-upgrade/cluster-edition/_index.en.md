...
explicitly uses `5101` and `5102` for the Coordinator link and management ports.

#### Deployer

The Deployer carries out package deployment and initialization requested by the Coordinator.
The manual example later in this page prepares a Deployer on each data-processing host.
...
such as comparing replicas, and are separate from application connection paths. Consider
redundant Brokers.
...
availability. Consider at least two members per group.
...
See [Standard and Cluster Differences](../../core-concepts/concepts-edition/#differences-standard-edition-cluster).
...
The file declares the nodes and their paths. Replace the example addresses with actual hosts.
...
| `cluster.hosts` | Host aliases and SSH addresses in `user@host` form |
...
| `cluster.defaults` | Role-specific home and port defaults |
...
Consider two Coordinators (Primary and Secondary), one or more Deployers, one Lookup master
and at least one monitor, two or more Brokers, and two Warehouses per replication group.

If several nodes of the same type share one host, explicitly give the additional nodes
different home paths and ports. Coordinator and Deployer management use `http_admin_port`.
...
Use the installation dry run to inspect SSH access, package availability, remote directory
...
| `deployer ... not found` | Unknown Deployer reference | Use a configured Deployer alias or host:port |
...
The command copies and extracts packages, generates node configurations and port settings,
initializes the Coordinator database, and registers nodes.
...
| Start and stop | Cluster commands | Individual node commands |
...
| Lightweight | Broker and Warehouse | Files needed for data processing |
...
Run this after the Coordinator has been installed and started:
...
before adding Broker or Warehouse nodes.
...
The Deployer creates the node configuration during `--add-node`. Choose the cluster link and
...
| `--dbs-path` | Broker/Warehouse data path; otherwise uses normal DBS_PATH behavior |
...
The Deployer generates the configuration during registration. Establish the cluster link,
