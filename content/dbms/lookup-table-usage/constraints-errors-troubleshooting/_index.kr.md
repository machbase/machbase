...
toc: true
---
LOOKUP 테이블의 제약 사항, 발생 가능한 오류, 문제 해결 방법을 다룬다.
...
row에 적용된다. 의도보다 많은 row가 변경·삭제되지 않도록 대상 범위를 먼저 확인한다.
...
- 일괄 변경은 조건을 더 좁히고 변경 전후 count를 확인한다.
- JSON 숫자 조건은 타입별 함수를 사용한다.
...
위와 같이 실행하면 JSON 컬럼을 primary key로 사용할 수 없다는 오류가 발생한다.

### 원인

Primary key는 row를 안정적으로 식별해야 한다. JSON 컬럼은 저장·조회·조건 검색·갱신에는
쓸 수 있지만 primary key 타입으로는 허용되지 않는다.

### 해결 방법

식별자는 별도 컬럼으로 분리하고 JSON은 일반 컬럼으로 둔다.
...
JSON 내부 값으로 조회해야 하면 JSON path 조건을 사용한다.
...
- JSON path 문자열은 작은따옴표(`'$.status'`)로 작성한다.
- 큰따옴표(`"$.status"`)는 SQL 식별자로 해석되어 컬럼 이름 오류가 발생할 수 있다.
...
LOOKUP 테이블은 소규모 참조 데이터에 최적화되어 있다. 건수가 수백만 건을 넘으면 성능이 저하될 수 있으므로 RDB 테이블을 고려한다.
...
- LOOKUP 테이블에 시계열 데이터를 저장하지 않는다. 계측값은 TAG 테이블을 사용한다.
- PRIMARY KEY 중복 삽입 시 오류가 발생한다.
- 테이블 전체를 교체해야 하면 DELETE 후 INSERT 또는 TRUNCATE + INSERT 패턴을 사용한다.
