...
SQL 문을 사용하여 데이터를 조회합니다. LOOKUP 테이블은 `PRIMARY KEY` 기반 조회, 일반 조건 조회, 다른 테이블과의 JOIN에 주로 사용합니다.
...
문자열 키를 사용하는 경우에도 같은 방식으로 조회합니다.
...
로그 이벤트의 코드값을 사람이 읽을 수 있는 라벨로 변환할 때도 사용합니다.
