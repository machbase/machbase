...
Machbase errors appear in machsql, driver exceptions, and server trace logs. Handle an
error by its `ERR-xxxxx` code rather than by matching the complete message string.

The complete catalog below preserves the NFX English message. `KEY` is the source
symbolic identifier. Tokens such as `%s`, `%d`, and `%llu` are replaced with runtime
values when the error is raised. The JSON manifest preserves the decoded source string;
the rendered table removes only semantically empty trailing whitespace.

The catalog is grouped into 1,000-code ranges on this page. Search by error code,
symbolic key, or message text. Causes, retryability, Edition scope, and remedies are not
inferred when they are absent from the source catalog.
...
The following 1,086 entries are generated from the `ERR_ID`, `KEY`, and `MSG_EN` fields in the Machbase 8.7.0 NFX error catalog.
...
Preserve the complete error, SQL or command, server and SDK versions, Edition, target
database, owner, and table. See [Troubleshooting](/dbms/troubleshooting/) and the
[operations diagnosis guide](/dbms/operations-configuration-recovery/diagnosis-observability/)
for symptom-based diagnostics.
