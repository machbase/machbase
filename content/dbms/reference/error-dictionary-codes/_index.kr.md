...
아래 전체 목록은 NFX 오류 카탈로그의 영문 메시지를 그대로 표시합니다. `KEY`는 source의
symbolic identifier이며 `%s`, `%d`, `%llu` 같은 token은 오류가 발생할 때 실제 값으로
치환됩니다. JSON manifest는 decoded 원문을 보존하고, 화면 표에서는 의미가 없는 줄 끝
공백만 제거합니다. 메시지 일부 문자열보다 `ERR-xxxxx` 코드를 기준으로 검색하십시오.

목록은 같은 페이지에서 1,000번 단위 범위로 나뉩니다. 브라우저 찾기 기능으로 오류 코드,
symbolic key 또는 메시지 일부를 검색할 수 있습니다. source에 없는 한국어 원인, retry 가능
여부와 Edition 범위는 자동으로 추정하지 않습니다.
