...
로그 테이블이 할당할 수 있는 최대 메모리 크기를 지정합니다. 서버가 이 값 이상의 메모리를
할당하면 사용량이 상한 아래로 줄어들 때까지 추가 할당이 대기할 수 있습니다. 운영 체제 예약분,
캐시, 인덱스 빌드와 쿼리 처리 공간을 포함한 전체 메모리 예산 안에서 조정합니다.
...
[Property](/dbms/reference/configuration/original-8-5-full/#property)와 별개로, Cluster Edition 에서만 사용 가능한 Property 를 정리합니다.
