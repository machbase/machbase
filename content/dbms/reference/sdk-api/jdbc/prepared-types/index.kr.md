---
type: docs
title: '17.7.2.1 PreparedStatement와 타입'
weight: 10
toc: true
---

PreparedStatement는 SQL을 서버에서 prepare한 뒤 파라미터만 바꾸어 반복 실행합니다.
Machbase JDBC는 표준 positional parameter와 이름 기반 확장 parameter를 제공합니다.
...
파라미터 index는 1부터 시작합니다. 0 또는 파라미터 개수를 초과하는 index에는
`SQLException`이 발생합니다. 숫자 precision은 저장 byte 수가 아니라 JDBC의 10진수
자릿수입니다. DATETIME은 `java.sql.Types.TIMESTAMP`로 매핑되지만 database type name은
`DATETIME`입니다.

## Named Bind Parameter

`:name` marker와 `MachPreparedStatement.setObject(String, Object)`는 Machbase 확장
...
- 이름은 대소문자를 구분합니다.
- 이름 기반 setter와 숫자 index setter를 한 statement에서 혼용하지 않습니다.
- SQL에 없는 이름 또는 named/positional 혼용에는 SQLState `07009`가 발생합니다.
- named bind를 지원하지 않는 이전 서버에는 SQLState `0A000`이 발생합니다.

이식성이 필요한 애플리케이션은 JDBC 표준인 `?`와 숫자 index setter를 사용합니다. 공통
...
DECIMAL과 NUMERIC은 `BigDecimal`을 사용하며 지정한 scale에 맞게 바인딩합니다.
...
unsigned 파라미터의 NULL은 ParameterMetaData를 기준으로 native NULL 값으로 변환됩니다.
