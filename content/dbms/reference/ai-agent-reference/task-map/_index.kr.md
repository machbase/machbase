...
| 태스크 | 수행 방법 | 참조 문서 |
|--------|-----------|-----------|
| 센서/시계열 데이터 저장 | TAG 테이블 생성 후 Append API 사용 | [TAG 테이블](../../../core-concepts/tag-table/), [Append](../../../application-integration/concepts-common/append/) |
| 로그/이벤트 데이터 저장 | LOG 테이블 생성 후 Append API 또는 INSERT 사용 | [LOG 테이블](../../../core-concepts/log-table/) |
| CSV/파일 일괄 입력 | machloader `-i` 옵션으로 파일 로드 | [machloader](../../../application-integration/tools/machloader/) |
| LOOKUP 마스터 데이터 관리 | LOOKUP 테이블 INSERT/UPDATE/DELETE | [LOOKUP 테이블](../../../core-concepts/lookup-table/) |
| 실시간 스트림 입력 | Collector 또는 Fluentd 플러그인 | [Collector](../../../application-integration/tools/collector/), [Fluentd](../../../application-integration/tools/fluentd/) |
...
| 시간 범위 집계 조회 | ROLLUP 테이블 활용 또는 `GROUP BY` | [ROLLUP](../../../core-concepts/rollup/) |
| 텍스트/로그 검색 | LOG 테이블에 `WHERE text LIKE` 또는 전문 검색 | [LOG 테이블](../../../core-concepts/log-table/) |
| 시계열 보간 / 시리즈 분석 | `SERIES BY` 절 사용 | [SELECT 문법](../../../reference/sql/syntax-dictionary-sql/select-syntax/) |
| 복수 태그 비교 조회 | `FROM TAG TABLE t WHERE name IN (...)` | [TAG 테이블](../../../core-concepts/tag-table/) |
| 집계 결과 빠른 조회 | ROLLUP 결과 테이블 SELECT | [ROLLUP](../../../core-concepts/rollup/) |

## 사용자 / 보안

| 태스크 | 수행 방법 | 참조 문서 |
|--------|-----------|-----------|
| 사용자 생성 | `CREATE USER username IDENTIFIED BY password` | [사용자 관리](../../../security-access-control/user-management/) |
| 권한 부여 | `GRANT privilege ON object TO user` | [GRANT/REVOKE](../../../security-access-control/privileges/grant-revoke/) |
| AUTH KEY 등록 | `CREATE AUTH KEY` + 공개키 파일 등록 | [AUTH KEY](../../../security-access-control/authentication-auth-key/) |
| 원격 접속 허용 | 설정 파일 `BIND_IP_ADDRESS`, `PORT_NO` 확인 및 방화벽 설정 | [원격 접속](../../../security-access-control/remote-access/) |
| 비밀번호 변경 | `ALTER USER username IDENTIFIED BY new_password` | [사용자 관리](../../../security-access-control/user-management/) |
