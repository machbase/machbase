...
```sql
EXEC ROLLUP_REBUILD(table_name, rollup_name, start_time, end_time)
```
...
| `rollup_name` | identifier | 재계산할 ROLLUP 이름 |
...
- 원본 데이터 수정(이상값 삭제 및 재입력) 후 실행해야 의미 있는 결과를 얻을 수 있습니다.
...
```sql
-- 이상 데이터 삭제 후 재입력
DELETE FROM sensor_tag WHERE name = 'TEMP-01'
