...
INET/HTTP 리스너가 바인드할 IP 주소를 지정합니다. `GRANT_REMOTE_ACCESS=1`이면 이 주소로 바인드하고, `GRANT_REMOTE_ACCESS=0`이면 루프백 주소에만 바인드합니다. `0.0.0.0`은 모든 인터페이스를 의미합니다.
...
Unix domain socket 파일의 경로를 설정합니다. 사용자가 설정하지 않았을 경우의 기본 값은 ?/conf/machbase-unix 입니다.
