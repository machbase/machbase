...
weight: 10
---
C/C++, Java, Python, .NET, Go, Node.js, REST API 등 여러 연동 경로 중 프로젝트에 가장 적합한 방식을 고르는 기준을 정리한다.
...
연동 방식을 선택할 때는 세 가지 관점에서 검토한다.

**개발 언어와 생태계**
프로젝트에서 사용하는 언어와 기존 라이브러리 생태계를 우선 고려한다. Java 프로젝트라면 JDBC, Python 데이터 파이프라인이라면 Python SDK가 자연스러운 선택이다.

**데이터 입력 성능**
초당 수만 건 이상의 대용량 시계열 데이터를 입력해야 한다면 Append API를 지원하는 CLI/ODBC, JDBC, Python SDK, .NET, Go native, Node.js 드라이버가 적합하다. REST API도 `/machbase` POST Append를 지원하지만 HTTP JSON 요청 단위로 동작하므로 네이티브 드라이버가 어려운 환경에서 우선 검토한다.

**운영 환경**
방화벽으로 인해 TCP 5656 포트를 사용할 수 없는 환경이라면 HTTP 기반 REST API를 검토한다. Windows 환경의 산업용 애플리케이션이라면 .NET 드라이버나 ODBC를 우선 고려한다.
...
프로젝트의 언어, 성능 요구사항, 운영 환경에 따라 적합한 연동 방식이 달라진다.
...
CLI/ODBC의 Append API를 권장한다. C/C++로 구현된 수집 에이전트가 직접 Machbase에 데이터를 밀어넣는 구조다.
...
JDBC, Python SDK, .NET, Go native, Node.js 드라이버의 Append API를 활용한다. Java 기반 Spring Boot 수집 서비스, Python 기반 데이터 파이프라인, Go/Node.js 기반 수집 에이전트에 적합하다.
...
어떤 드라이버든 충분히 처리할 수 있다. Prepared statement와 connection pool을 조합하면 된다.

#### 저빈도 조회 위주 (관리, 대시보드)

REST API만으로 충분하다. 별도 드라이버 설치 없이 HTTP 요청으로 데이터를 조회할 수 있다.
...
> `-` 는 미지원 또는 해당 없음을 의미한다. REST API 인증은 DB 포트의 AUTH KEY challenge가 아니라 `HTTP_AUTH` 기반 Basic Authentication을 사용한다. 각 드라이버의 최신 지원 현황은 14장 레퍼런스를 참조한다.

### Append API 지원 여부가 중요한 이유

Machbase는 시계열 데이터베이스이므로 대부분의 운영 환경에서 **초당 수천 건 이상의 쓰기**가 발생한다. Append API는 트랜잭션 없이 전용 세션이나 요청 형식으로 데이터를 입력하므로, 일반 INSERT 반복 실행 대비 훨씬 높은 쓰기 처리량을 제공한다.

대용량 수집이 요구사항에 포함된다면 Append API를 지원하는 드라이버를 반드시 선택해야 한다.

자세한 동작 원리는 [Append API와 Batch INSERT](/dbms/application-integration/concepts-common/#append-api-batch)를 참조한다.

<a id="distinction-sdk-api-canonical-owner"></a>

## SDK/API/도구별 canonical owner 구분

Machbase 문서는 역할에 따라 두 곳으로 나뉜다.
...
처음 Machbase에 연동하는 개발자가 읽는 장이다. 드라이버 선택 근거, 공통 개념(인증, 타임존, 트랜잭션 범위 등), 그리고 각 드라이버의 기본 사용 패턴을 다룬다. 코드 예제는 개념 설명 수준의 축약 예시다.

**14장 → "정확히 어떻게 동작하는가"**

특정 함수나 옵션의 정확한 동작을 확인해야 할 때 참조한다. 함수 시그니처, 반환값, 에러 코드, 옵션 파라미터의 전체 목록이 있다.

### 외부 도구의 문서 위치

Machbase와 연동하는 외부 도구는 8장의 외부 도구 섹션에서 다룬다.
...
> 외부 도구 자체의 상세 사용법은 각 도구의 공식 문서를 참조한다. Machbase 문서에서는 Machbase와의 연동 설정에만 집중한다.
