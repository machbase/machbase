...
weight: 60
---
각 SDK는 지원하는 기능 범위가 다르다. 애플리케이션 요구사항에 맞는 SDK를 선택하려면 아래 기능별 지원 여부를 확인해야 한다.
...
> **참고**: TAG 테이블과 LOG 테이블은 Append-only 구조로, 트랜잭션이 필요 없는 고속 스트리밍 쓰기에 최적화되어 있다.
...
**Append API**는 대량 데이터를 고속으로 삽입하기 위한 전용 프로토콜이다. 일반 `INSERT` SQL 대비 수십 배 이상의 처리량을 제공하며, 시계열 데이터 수집에서 핵심적으로 사용된다.
...
REST API를 통한 Append는 JSON 본문에 테이블 이름과 행 배열을 전달한다.
...
> **참고**: RDB(Lookup) 테이블에 대한 Append API 사용은 지원되지 않는다. RDB 테이블에는 일반 `INSERT` SQL을 사용한다.

<a id="support-scope-sdk-auth-key"></a>

## SDK별 AUTH KEY 지원 범위 안내

Machbase는 사용자 이름/비밀번호 방식 외에 **AUTH KEY challenge 인증**을 지원한다.
서버에는 사용자의 공개키를 등록하고, 클라이언트는 개인키 파일로 서버 challenge에
서명한다. 비밀번호를 연결 문자열에 넣지 않고 키 기반으로 접속할 때 사용한다.

### AUTH KEY 인증 방식 개요

1. 서버 사용자에 공개키를 AUTH KEY로 등록한다.
2. 클라이언트가 `AUTH_MODE=CHALLENGE`로 연결을 요청한다.
3. 서버가 challenge 값을 보낸다.
4. 클라이언트가 `AUTH_KEY_FILE` 개인키로 challenge에 서명한다.
5. 서버가 등록된 공개키로 서명을 검증하고 연결을 허용한다.

지원되는 서명 방식은 `ECDSA`, `RSA_PKCS1_V15`, `RSA_PSS`이다. 키 파일만으로 방식을
추론할 수 있는 경우에는 `AUTH_SIG_SCHEME`을 생략할 수 있다.

### AUTH KEY 등록과 조회

사용자를 생성할 때 공개키를 함께 등록할 수 있다.
...
기존 사용자에 AUTH KEY를 추가하는 것도 가능하다.
...
등록된 키는 `V$USER_AUTH_KEYS`에서 확인한다.
...
키를 비활성화, 활성화, 삭제할 때는 `key_id`를 지정한다.
...
`AUTH_MODE=CHALLENGE`를 지정하면 `AUTH_KEY_FILE`이 필수다. 잘못된 파일 경로,
키와 맞지 않는 `AUTH_SIG_SCHEME`, 등록되지 않은 사용자 키는 연결 실패로 처리된다.
...
인증 방식이다. REST API(기본 5657)는 별도의 로그인 토큰 발급 엔드포인트나 Bearer
토큰 교환 방식으로 AUTH KEY를 처리하지 않는다. REST API 인증은 `machbase.conf`의
`HTTP_AUTH` 설정에 따라 Basic Authentication을 사용한다.

### 보안 권장 사항

1. 개인키 파일은 파일 권한을 제한하고 애플리케이션 설정 또는 시크릿 관리 도구로
   배포한다.
2. 공개키는 사용자별로 등록하고, 사용하지 않는 키는 `DEACTIVATE` 또는 `DROP`한다.
3. `VALID_BEFORE`를 설정해 키 만료 시점을 운영 정책에 맞게 관리한다.
...
각 SDK가 지원하는 트랜잭션, Prepared Statement, Parameter Binding 기능을 정리한다.

> TAG 및 LOG 테이블은 append-only 구조로 트랜잭션이 필요하지 않다. 트랜잭션은 **RDB 테이블**에서만 의미가 있다.
...
RDB 테이블에서만 COMMIT/ROLLBACK이 유효하다. 나머지 테이블 유형은 삽입 즉시 영구 저장된다.
...
반복 실행할 쿼리를 미리 파싱·컴파일하여 성능을 높인다. SQL 인젝션 방지 효과도 있다.

Python `machbaseAPI` DB-API 스타일 커서는 별도 `prepare()` 메서드를 제공하지 않는다.
같은 SQL 문자열과 `%s` 파라미터를 반복 호출하는 방식이다.
...
파라미터 바인딩 시 DATETIME 타입은 **나노초 정수**로 전달하는 것을 권장한다.
...
각 SDK에서 NULL 값을 바인딩하는 방법은 다음과 같다.
...
상세 내용은 각 [드라이버별 가이드](/dbms/application-integration/guide-drivers/)를 참고한다.

<a id="sdk"></a>

## 전체 SDK 기능 지원표 링크 (canonical owner: 14. 레퍼런스)

Machbase Neo가 지원하는 SDK 목록과 각 SDK의 상세 레퍼런스 위치를 안내한다. 기능별 상세 API 레퍼런스는 **14장 레퍼런스** 섹션을 참고한다.
...
#### ODBC / CLI (C/C++)
네이티브 C 인터페이스로 가장 낮은 레이턴시와 최대 처리량을 제공한다. 임베디드 시스템이나 성능이 중요한 데이터 수집 에이전트에 적합하다.
...
#### JDBC (Java)
표준 JDBC 4.x 인터페이스를 구현하여 Spring, Hibernate, MyBatis 등 Java 생태계 프레임워크와 호환된다.
...
#### Python
Python DB-API 2.0(PEP 249) 인터페이스를 제공한다.
...
#### .NET (C#)
ADO.NET 인터페이스를 구현하여 .NET 애플리케이션에서 표준적으로 사용할 수 있다.
...
#### Node.js
JavaScript / TypeScript 환경에서 사용한다.

- Promise 기반 비동기 API
- `appendBatch` / `appendOpen`을 통한 LOG/TAG Append API 지원

#### REST API
HTTP 기반으로 언어·프레임워크에 독립적이다. Machbase Neo의 HTTP 서버(포트 5657)에 직접 요청을 보낸다.
...
각 기능의 SDK별 지원 여부는 아래 페이지를 참고한다.
