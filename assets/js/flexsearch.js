// Based on Hextra v0.12.3. Local changes support IME/paste input and await index loading.
...
(function () {
  ...
  for (const el of inputElements) {
    ...
    el.addEventListener('compositionstart', () => composingInputs.add(el));
  ...
  function handleInputChange(e) {
    ...
    if (!e.isComposing && !composingInputs.has(e.target)) runSearch(e);
  }

  function runSearch(event) {
    search(event).catch(error => console.warn('Unable to load search index', error));
  }

  function ensureIndex() {
    if (!indexPromise) {
      indexPromise = preloadIndex().catch(error => {
  ...
  function getActiveSearchElement() {
    ...
    const wrapper = inputs.find(el => el.contains(document.activeElement)) || inputs[0];
    return wrapper ? {
      wrapper,
  ...
  function handleKeyDown(e) {
    if (e.isComposing || composingInputs.has(e.target) || e.keyCode === 229) return;
    const { inputElement } = getActiveSearchElement();
    if (!inputElement) return;
  ...
  function init(e) {
    ...
    ensureIndex().catch(error => console.warn('Unable to load search index', error));
  }
  ...
  async function preloadIndex() {
    ...
    for (const route in data) {
      ...
      for (const heading in data[route].data) {
        ...
        const hash = separator < 0 ? heading : heading.slice(0, separator);
        const text = separator < 0 ? '' : heading.slice(separator + 1);
  ...
  async function search(e) {
    ...
    await ensureIndex();
    if (query !== e.target.value || document.activeElement !== e.target || composingInputs.has(e.target)) return;
    const { resultsElement } = getActiveSearchElement();
    if (!resultsElement) return;
